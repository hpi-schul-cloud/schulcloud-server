...
module.exports = {
	up: async function up() {
		info(
			'Setting up maintenance mode for schools from Cluster 4 or with no federalState assigned',
		);
		await connect();

		const nextSchoolYearId = await YearModel.findOne({ name: '2020/21' })
			.select('_id')
			.lean()
			.exec();
		...
		info(
			`Migrating schools in ${federalStateIds.length}: ${federalStateIds} or with no federal state assigned`,
		);
		...
		info(
			`Migration result of LDAP Schools: ${resultLdapSchools.nModified} schools updated`,
		);
		...
		info(
			`Migration result of Non-LDAP Schools in: ${resultNonLdapSchools.nModified} schools updated`,
		);
	...
	down: async function down() {
		await connect();
		info(
			'Disabling Maintenance mode for schools from Cluster 4 or with no federalState assigned',
		);
		...
		const currentSchoolYearId = await YearModel.findOne({ name: '2019/20' })
			.select('_id')
			.lean()
			.exec();
