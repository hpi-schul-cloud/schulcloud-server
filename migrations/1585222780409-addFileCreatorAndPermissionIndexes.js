...
module.exports = {
	up: async function up() {
		...
		const [
			filesWithOwnerPermissions,
			userOwnedFilesWithInvalidPermissions,
		] = await Promise.all([
		...
		const filesToUpdate =
			filesWithOwnerPermissions + userOwnedFilesWithInvalidPermissions;
		info(
			`${allFiles} files exist. ${filesToUpdate} files have creator information.`,
		);
		...
		await FileModel.updateMany(
			...
			[
				{
					$set: {
						creator: { $arrayElemAt: ['$permissions.refId', 0] },
					},
				},
			],
		);
