...
module.exports.up = async function courseStudentUp(next) {
	await connect();
	const courseStudentRole = await roleModel
		.findOne({ name: 'courseStudent' })
		.exec();
	...
	toUpdateRoles.forEach(async (role) => {
		await roleModel
			.findOneAndUpdate(
				{ name: role },
				{ roles: [courseStudentRole._id.toString()] },
			)
			.exec();
	});

	const courseTeacherRole = await roleModel
		.findOne({ name: 'courseTeacher' })
		.exec();
	await roleModel
		.findOneAndUpdate(
			{ name: 'courseAdministrator' },
			{ roles: [courseTeacherRole._id.toString()] },
		)
		.exec();
