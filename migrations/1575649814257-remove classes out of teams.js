	...
	up: async function up() {
		...
		for (const team of teams) {
			try {
				...
				const classIds = team.classIds.map((classId) =>
					String(classId),
				);
				info(
					`following classIds will be removed: ${JSON.stringify(
						classIds,
					)}`,
				);
				team.classIds = [];
		...
		const teamsAfter = await Teams.find({
			'classIds.0': { $exists: true },
		}).exec();
		...
		if (teamsAfter.length !== 0) {
			throw new Error(
				`there were ${teamsAfter.length} teams not updated successfully!`,
			);
		}
