	...
	up: async function up() {
		...
		while (looped < amount) {
			...
			try {
				...
				error(
					`Removing user.consent.id's between ${skip * limit} and ${
						skip * limit + limit
					} failed,
			...
			try {
				await Promise.all(
					users.map(async (user) => {
						...
						const consent = { ...user.consent };
						return oldUser
							.findByIdAndUpdate(user._id, { $set: { consent } })
							.exec();
					}),
				);
				...
				error(
					`Inserting user.consent.id's between ${skip * limit} and ${
						skip * limit + limit
					} failed,
but will go on with next loop: ${err.message}\n ${err}`,
				);
