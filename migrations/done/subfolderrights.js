...
const name =
	'Add permission for students to create files in course subfolders.';
...
const run = async (dry) => {
	...
	const { _id: studentRoleId } = await RoleModel.findOne({ name: 'student' })
		.lean()
		.exec();
	...
	const promises = courseFolders.map((dir) => {
		...
		const studentPermissions = permissions.find(
			({ refId }) => refId && refId.equals(studentRoleId),
		);
