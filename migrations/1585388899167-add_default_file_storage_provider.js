...
const StorageProvider = mongoose.model(
	'storageProviderMigration',
	storageProviderSchema,
	'storageproviders',
);
...
module.exports = {
	up: async function up() {
		...
		if (!process.env.AWS_SECRET_ACCESS_KEY) {
			warning(
				"No AWS config found. Migration will be successful but won't change anything!",
			);
		} else {
			if (!Configuration.has('S3_KEY')) {
				throw new Error(
					'You need to set process.env.S3_KEY to encrypt the old key!',
				);
			}
			...
			try {
				...
				unassignedSchools = await School.countDocuments({
			...
			const secretAccessKey = await CryptoJS.AES.encrypt(
			...
			info(
				`Created default storage provider (${process.env.AWS_ENDPOINT_URL}) for all existing schools.`,
			);
		}
