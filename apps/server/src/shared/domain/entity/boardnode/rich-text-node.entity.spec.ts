...
import { RichTextNode } from './rich-text-node.entity';
import { BoardDoBuilder, BoardNodeType } from './types';

describe(RichTextNode.name, () => {
	describe('when trying to create a rich text element', () => {
		...
		it('should create a FileElementNode', () => {
			...
			const element = new RichTextNode(elementProps);

			expect(element.type).toEqual(BoardNodeType.RICH_TEXT);
		});
	});

	describe('useDoBuilder()', () => {
		const setup = () => {
			const element = new RichTextNode({ text: 'Test', inputFormat: InputFormat.RICH_TEXT_CK5 });
			const builder: DeepMocked<BoardDoBuilder> = createMock<BoardDoBuilder>();
