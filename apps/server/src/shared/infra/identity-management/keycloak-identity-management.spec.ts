...
describe('KeycloakIdentityManagement', () => {
	...
	it('should return null if account create fails', async () => {
		...
		const retFirstTry = await idm.createAccount(testAccount, testAccountPassword);
		expect(retFirstTry).toBeNull();
		...
		const retSecondTry = await idm.createAccount(testAccount, testAccountPassword);
		expect(retSecondTry).toBeNull();
	...
	it('should return null if account can not be updated', async () => {
		const accountId = 'user-1-id';
		const testAccount = { userName: 'test', email: 'test@test.test' };
	...
	it('should return null if account can not be deleted', async () => {
		const accountId = 'user-1-id';
	...
	it('should fail to update a non-existing accounts password', async () => {
