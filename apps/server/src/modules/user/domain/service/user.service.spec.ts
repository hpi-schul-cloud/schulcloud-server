...
describe('UserService', () => {
	...
	describe('markUserAsDeleted', () => {
		describe('for a single user', () => {
			const setup = () => {
				...
				return { userId, deleteAt };
			};

			it('should call markDeleted with userId and deleteAt', async () => {
				const { userId, deleteAt } = setup();

				await service.markUserAsDeleted(userId, deleteAt);

				expect(userRepo.markDeleted).toHaveBeenCalledWith([userId], deleteAt);
			});
		});
		...
		describe('for multiple users', () => {
			const setup = () => {
				...
				return { userIds, deleteAt };
			};

			it('should call markDeleted with userIds and deleteAt', async () => {
				const { userIds, deleteAt } = setup();

				await service.markUserAsDeleted(userIds, deleteAt);

				expect(userRepo.markDeleted).toHaveBeenCalledWith(userIds, deleteAt);
			});
		});
	});
	...
	describe('restoreDeletedUser', () => {
		describe('for a single user', () => {
			...
			it('should call markDeleted with userId and null', async () => {
				const { userId } = setup();

				await service.restoreDeletedUser(userId);

				expect(userRepo.markDeleted).toHaveBeenCalledWith([userId], null);
			});
		});
		...
		describe('for multiple users', () => {
			...
			it('should call markDeleted with userIds and null', async () => {
				const { userIds } = setup();

				await service.restoreDeletedUser(userIds);

				expect(userRepo.markDeleted).toHaveBeenCalledWith(userIds, null);
			});
		});
	});
