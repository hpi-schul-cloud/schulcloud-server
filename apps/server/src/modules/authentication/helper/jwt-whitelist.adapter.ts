...
export class JwtWhitelistAdapter {
	constructor(@Inject(VALKEY_CLIENT) private readonly valkeyClient: ValkeyClient) {}
	...
	public async addToWhitelist(accountId: string, jti: string): Promise<void> {
		...
		await this.valkeyClient.set(redisIdentifier, JSON.stringify(redisData), 'EX', expirationInSeconds);
	}
	...
	public async removeFromWhitelist(accountId: string, jti?: string): Promise<void> {
		...
		if (jti) {
			...
			keys = await this.valkeyClient.keys(redisIdentifier);
		}

		const deleteKeysPromise: Promise<number>[] = keys.map((key: string): Promise<number> => this.valkeyClient.del(key));
