...
export class LocalStrategy extends PassportStrategy(Strategy) {
	...
	async validate(username?: string, password?: string): Promise<ICurrentUser> {
		...
		const accountUserId = GuardAgainst.nullOrUndefined(
			account.userId,
			new Error(`login failing, because account ${account.id ? account.id : ''} has no userId`)
		);
		const user = await this.userRepo.findById(accountUserId, true);
		const currentUser = CurrentUserMapper.userToICurrentUser(account.id ? account.id : '', user, false);
		return currentUser;
	}
