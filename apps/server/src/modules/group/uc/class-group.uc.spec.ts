	...
	describe('findAllClasses', () => {
		...
		describe('when accessing as a normal user', () => {
			const setup = () => {
				...
				const groupSubstitutionTeacherRole = roleDtoFactory.buildWithId({
					id: 'group-substitution-teacher-role-id',
					name: RoleName.GROUPSUBSTITUTIONTEACHER,
				});
				...
				const group = groupFactory.build({
					name: 'B',
					users: [{ userId: teacherUser.id, roleId: teacherRole.id }],
				...
				userService.findByIdOrNull.mockResolvedValue(teacherUserDo);
		...
		describe('when accessing as an admin', () => {
			...
			describe('when sorting by external source name in descending order', () => {
				it('should return all classes sorted by external source name in descending order', async () => {
					const { adminUser, clazz, group, groupWithSystem, system, schoolYear, teacherUser } = setup();
