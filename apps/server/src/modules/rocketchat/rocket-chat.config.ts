...
export class RocketChatConfig extends RocketChatPublicApiConfig {
	...
	@IsString()
	@ValidateIf((config: RocketChatConfig) => (!config.adminUser || !config.adminPassword) && config.rocketChatServiceEnabled)
	...
	@IsString()
	@ValidateIf((config: RocketChatConfig) => (!config.adminId || !config.adminToken) && config.rocketChatServiceEnabled)
