import { type AnyBoardNode, type Card } from '../../domain';
...
export class CardResponseMapper {
	public static mapToResponse(card: Card): CardResponse {
		const result = new CardResponse({
			id: card.id,
			title: card.title,
			backgroundColor: card.backgroundColor,
			height: card.height,
			elements: card.children.map((element) => ContentElementResponseFactory.mapToResponse(element as AnyBoardNode)),
