...
import { Action, AuthorizationService, PermissionContextService } from '@modules/authorization';
import {
	BadRequestException,
	ForbiddenException,
	NotFoundException,
...
describe(SubmissionItemUc.name, () => {
	...
	// 			permissionContextService.resolvePermissions.mockImplementation(
	// 				(userId_: User['id'], contextReference: EntityId) =>
	// 					new Promise((resolve) => {
	// 						if (contextReference === submissionContainerEl.id) {
	// 							resolve([PermissionCrud.READ]);
	// 						} else if (contextReference === submissionItem1.id) {
	// 							resolve([PermissionCrud.READ]);
	// 						} else if (contextReference === submissionItem2.id) {
	// 							resolve([]);
	// 						} else {
	// 							resolve([]);
	// 						}
