...
import { DrawingDoRepo } from '../repo';
import { DrawingDoAuthorizableService } from '.';

describe(DrawingDoAuthorizableService.name, () => {
	let module: TestingModule;
	let service: DrawingDoAuthorizableService;
	let drawingDoRepo: DeepMocked<DrawingDoRepo>;
	...
	beforeAll(async () => {
		module = await Test.createTestingModule({
			providers: [
				...
				{
					provide: DrawingDoRepo,
					useValue: createMock<DrawingDoRepo>(),
				},
				{
					provide: CourseRepo,
		...
		service = module.get(DrawingDoAuthorizableService);
		drawingDoRepo = module.get(DrawingDoRepo);
	...
	describe('findById', () => {
		describe('when finding a board domainobject', () => {
			const setup = () => {
				...
				drawingDoRepo.findById.mockResolvedValue(columnBoard);
			...
			it('should call the repository', async () => {
				...
				expect(drawingDoRepo.findById).toHaveBeenCalledWith(columnBoardId, 1);
			});
	...
	describe('getBoardAuthorizable', () => {
		describe('when having an empty board', () => {
			...
			it('should return an empty usergroup', async () => {
				const { board } = setup();
				drawingDoRepo.findById.mockResolvedValueOnce(board);
		...
		describe('when having a board with a teacher and some students', () => {
			...
			it('should return the teacher and the students with correct roles', async () => {
				...
				expect(userPermissions[studentIds[0]]).toEqual([BoardRoles.EDITOR]);
				expect(userPermissions[studentIds[1]]).toEqual([BoardRoles.EDITOR]);
				expect(userPermissions[studentIds[2]]).toEqual([BoardRoles.EDITOR]);
			});
			...
			it('should return the parentDo', async () => {
				...
				drawingDoRepo.findParentOfId.mockResolvedValueOnce(column);
		...
		describe('when trying to create a boardDoAuthorizable on a column without a columnboard as root', () => {
			const setup = () => {
				...
				const students = userFactory.buildListWithId(3);
				const column = columnFactory.build();
				drawingDoRepo.findById.mockResolvedValueOnce(column);
