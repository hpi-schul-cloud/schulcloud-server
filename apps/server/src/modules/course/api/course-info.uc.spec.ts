...
import { Course, CourseDoService, CourseFilter, CourseSortProps, CourseStatus } from '../domain';
	...
	describe('getCourseInfo', () => {
		describe('when calling getCourseInfo', () => {
			...
			it('should call with non-default options and filter', async () => {
				...
				const filter: CourseFilter = { schoolId: school.id, status: CourseStatus.ARCHIVE, withoutTeacher: true };
				const options: IFindOptions<Course> = {
					order: {
						name: SortOrder.asc,
					},
