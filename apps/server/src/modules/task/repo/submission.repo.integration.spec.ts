...
describe('submission repo', () => {
	...
	describe('findGradedByUserId', () => {
		it('should return only graded submissions', async () => {
			const helper = new TaskTestHelper();
			const student = helper.getFirstUser() as UserTaskInfo;

			const task = helper.createTask();
			const submission1 = helper.createSubmission(task);
			submission1.grade = 100;
			const submission2 = helper.createSubmission(task);

			await em.persistAndFlush([student, task, submission1, submission2]);

			const [result, count] = await repo.findGradedByUserId(student.id);
		...
		it('should return submissions where the user is a team member', async () => {
			...
			const [result, count] = await repo.findGradedByUserId(students[0].id);
			expect(count).toEqual(1);
			expect(result[0].teamMembers[0].firstName).toEqual(students[0].firstName);
		});
