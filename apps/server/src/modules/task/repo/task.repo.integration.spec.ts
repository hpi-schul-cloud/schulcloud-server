...
describe('TaskRepo', () => {
	...
	describe('findAllByParentIds', () => {
		...
		describe('find by creator', () => {
			it('should find tasks by creatorId', async () => {
				...
				await em.persist([task1, task2]).flush();
				em.clear();
		...
		describe('find by teacher and courses', () => {
			it('should find tasks by teacher and courses', async () => {
				...
				await em.persist([task1, task2, task3, task4, task5, task6]).flush();
				em.clear();
			...
			it('should not find private tasks created by other users', async () => {
				...
				await em.persist([user, task]).flush();
				em.clear();
		...
		describe('order', () => {
			it('should order by dueDate asc', async () => {
				...
				await em.persist([task1, task2, task3, task4]).flush();
				em.clear();
	...
	describe('findAllFinishedByParentIds', () => {
		...
		it('should populate submissions of task', async () => {
			...
			await em.persist([task, submission]).flush();
			em.clear();
	...
	describe('deleteAllPrivateTasksByTeacherId', () => {
		it('should delete only private tasks for the given teacherId', async () => {
			...
			await em.persist([privateTask1, privateTask2, publicTask, otherTeacherTask]).flush();
	...
	describe('findBySingleParent', () => {
		it('should find all published tasks in course', async () => {
			...
			await em.persist([user, course, task]).flush();
	...
	describe('findByOnlyCreatorId', () => {
		describe('when searching by creatorId', () => {
			const setup = async () => {
				...
				await em.persist([task, taskWithCourse]).flush();
				em.clear();
	...
	describe('findByCreatorIdWithCourseAndLesson', () => {
		describe('when searching by creatorId', () => {
			const setup = async () => {
				...
				await em.persist([task, taskWithCourse, taskWithCourseAndLesson]).flush();
				em.clear();
	...
	describe('findByUserIdInFinished', () => {
		describe('when searching by userId', () => {
			const setup = async () => {
				...
				await em.persist([taskWithFinished, taskWithoutFinished]).flush();
				em.clear();
