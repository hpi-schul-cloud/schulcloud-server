...
export class FileDtoBuilder {
	private static build(name: string, data: Readable, size: number, mimeType: string): FileDto {
		const file = new FileDto({ name, data, size, mimeType });

		return file;
	}

	public static buildFromRequest(fileInfo: FileInfo, request: Request, buffer: Readable): FileDto {
