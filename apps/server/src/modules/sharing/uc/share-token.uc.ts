...
export class ShareTokenUC {
	...
	private async checkParentWritePermission(userId: EntityId, payload: ShareTokenPayload) {
		...
		await this.authorizationService.checkIfAuthorizedByReferences(userId, allowedParentType, payload.parentId, {
			action: Action.write,
			requiredPermissions,
		});
	}
	...
	private async checkContextReadPermission(userId: EntityId, context: ShareTokenContext) {
		...
		await this.authorizationService.checkIfAuthorizedByReferences(userId, allowedContextType, context.contextId, {
			action: Action.read,
			requiredPermissions: [],
		});
	}
