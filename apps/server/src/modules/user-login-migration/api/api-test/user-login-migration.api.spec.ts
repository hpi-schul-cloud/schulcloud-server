	...
	describe('[GET] /user-login-migrations', () => {
		describe('when data is given', () => {
			const setup = async () => {
				...
				const targetSystem: SystemEntity = systemEntityFactory.withOauthConfig().buildWithId({ alias: 'moin.schule' });
	...
	describe('[POST] /close', () => {
		describe('when the user login migration is running', () => {
			const setup = async () => {
				...
				await em
					.persist([sourceSystem, targetSystem, school, adminAccount, adminUser, userLoginMigration, migratedUser])
					.flush();
				em.clear();
		...
		describe('when no user has migrate', () => {
			const setup = async () => {
				...
				await em
					.persist([sourceSystem, targetSystem, school, adminAccount, adminUser, userLoginMigration, user])
					.flush();
				em.clear();
		...
		describe('when "forceExtendedMode" is false', () => {
			describe('when forcing a school to migrate', () => {
				const setup = async () => {
					...
					await em
						.persist([sourceSystem, targetSystem, school, superheroAccount, superheroUser, adminAccount, adminUser])
						.flush();
					em.clear();
