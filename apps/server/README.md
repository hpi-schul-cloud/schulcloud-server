...
You can add these secrets by putting them into your env vars. E.g. if you add `SANIS_CLIENTID=<actual secret>` into your .env file, the secret will be written into the db, when you run the database setup. It is recommende to add the env var `AES_KEY` as well to encrypt those secrets.<br>
The real secrets can be found in the password store.<br>

While exporting the systems to JSON the secrets will be replaced by placeholders follwing the pattern `<capitalized alias>_<capitalized property name>`. So the system with alias "sanis" and the secret property "clientId" will be replaced by "SANIS_CLIENTID"
