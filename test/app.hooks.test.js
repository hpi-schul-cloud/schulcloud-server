...
describe('Sanitization Service', function sanitizationServiceTest() {
	...
	it('POST /news (Sanitization)', () => {
		const postBody = {
			...
			content: '<p>SanitizationTest<script>alert("test);</script><a href="javascript:test();'
			+ '">SanitizationTest</a></p>äöüß§$%/()=',
		};
	...
	it('DELETE /news (Sanitization)', () => newsService.remove(
		currentUsedId, { payload: { userId: '0000d213816abba584714c0a' } },
	)
		.then((result) => {
	...
	it('POST /helpdesk (Sanitization)', () => {
		const postBody = {
			...
			type: 'problem',
			currentState: '<p>SanitizationTest<script>alert("test);</script><a href="javascript:test();'
			+ '">SanitizationTest</a></p>äöüß§$%/()=',
			targetState: '<p>SanitizationTest<script>alert("test);</script><a href="javascript:test();'
			+ '">SanitizationTest</a></p>äöüß§$%/()=',
			category: 'dashboard',
			schoolId: '0000d186816abba584714c5f',
			// schoolId: '5836bb5664582c35df3bc000'
	...
	it('DELETE /helpdesk (Sanitization)', () => helpdeskService.remove(
		currentUsedId, { payload: { userId: '0000d213816abba584714c0a' } },
	)
		.then((result) => {
			expect(result).to.not.be.undefined;
	...
	it('POST /courses and /lessons (Sanitization)', () => {
		const postBody = {
			...
			description: '<p>SanitizationTest<script>alert("test);</script><a href="javascript:test();'
			+ '">SanitizationTest</a></p>äöüß§$%/()=',
			color: '#d32f22',
			...
			contents: [
				{
					...
					content: {
						text: '<p>SanitizationTest<script>alert("test);</script><a href="javascript:test();'
						+ '">SanitizationTest</a></p>äöüß§$%/()=',
					},
				},
			],
	...
	it('DELETE /lessons (Sanitization)', () => lessonService.remove(
		currentLessonId, { payload: { userId: '0000d213816abba584714c0a' } },
	)
		.then((result) => {
			expect(result).to.not.be.undefined;
			expect(result.name).to.equal('SanitizationTest äöüß§$%/()=');
	...
	it('DELETE /courses (Sanitization)', () => courseService.remove(
		currentUsedId, { payload: { userId: '0000d213816abba584714c0a' } },
	)
		.then((result) => {
			expect(result).to.not.be.undefined;
