...
describe('Sanitization Hook', () => {
	...
	it('hook does not sanitizes specified attributes', () => {
		...
		const data = {
			...
			title:
				'<script>alert("test");</script>SanitizationTest äöüß§$%/()=',
			content,
		};
	...
	it('sanitize in news, example', () => {
		...
		expect(result.content).to.equal(
			'<p>SanitizationTest<a>SanitizationTest</a></p>äöüß§$%/()=',
		);
	});
	...
	it('sanitize in news, example 2', () => {
		const data = {
			schoolId: '0000d186816abba584714c5f',
			title:
				'<script>alert("test");</script><b></b><i></i><img src="bla" />',
			content: 'a',
		};
	...
	it('sanitize in helpdesk, example 1', () => {
		...
		expect(result.currentState).to.equal(
			'SanitizationTestSanitizationTestäöüß§$%/()=',
		);
		expect(result.targetState).to.equal(
			'SanitizationTestSanitizationTestäöüß§$%/()=',
		);
	...
	it('sanitize in submissions, avoid img onerror attribute', () => {
		const data = {
			comment:
				'<img onerror="window.location = \'google.com\'" src="x" />',
		};
		...
		expect(
			result.comment,
			'onerror attribute removed from img tag',
		).to.equal('<img src="x" />');
	});
	...
	it('sanitize in submissions, avoid js in href', () => {
		...
		expect(result.comment, 'js removed from a href').to.equal(
			'<a>Link</a>',
		);
	});
	...
	it('sanitize in lessons, example 1', () => {
		...
		expect(result.name).to.equal('SanitizationTest äöüß§$%/()=');
		expect(result.contents[0].content.text).to.equal(
			'<p>SanitizationTest<a>SanitizationTest</a></p>äöüß§$%/()=',
		);
		expect(result.schoolId).to.equal('0000d186816abba584714c5f');
