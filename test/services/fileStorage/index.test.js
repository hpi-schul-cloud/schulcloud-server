...
describe('fileStorage services', () => {
	...
	after(async () => {
		...
		const promises = [
			...fixtures.teams.map((_) =>
				teamsModel.findByIdAndRemove(_._id).exec(),
			),
			...fixtures.schools.map((_) =>
				schoolModel.findByIdAndRemove(_._id).exec(),
			),
			...fixtures.files.map((_) =>
				FileModel.findByIdAndRemove(_._id).exec(),
			),
			...fixtures.users.map((_) =>
				userModel.findByIdAndRemove(_._id).exec(),
			),
			...fixtures.roles.map((_) =>
				RoleModel.findByIdAndRemove(_._id).exec(),
			),
			...fixtures.courses.map((_) =>
				courseModel.findByIdAndRemove(_._id).exec(),
			),
		];
	...
	describe('file service', () => {
		...
		after((done) => {
			const promises = created.map((id) =>
				FileModel.findByIdAndRemove(id),
			);
		...
		it('should create a course file object', (done) => {
			...
			fileStorageService
				.create(
					{ owner: '0000dcfbfb5c7a3f00bf21ac', ...params },
					context,
				)
				.then((res) => {
					// eslint-disable-next-line eqeqeq
					const isEqual = Object.keys(params).every(
						(key) => params[key].toString() == res[key].toString(),
					);
					const {
						_id,
						isDirectory,
					...
					expect(
						permissions[0]._id.equals('0000d224816abba584714c8e'),
					).to.be.true;
		...
		it('should create a team file object', (done) => {
			...
			fileStorageService
				.create(
					{ owner: '5cf9303bec9d6ac639fefd42', ...params },
					context,
				)
				.then((res) => {
					// eslint-disable-next-line eqeqeq
	...
	describe('signed url service', () => {
		...
		it('return a signed url for updating blobs a file', (done) => {
			...
			signedUrlService
				.patch('5ca613c4c7f5120b8c5bef33', {}, context)
				.then(({ url }) => {
					expect(url).to.be.equal('https://something.com');
					done();
				});
		...
		it('allows to override the internal file name with a request specific one', async () => {
			const context = setContext('0000d224816abba584714c8e');
			const requestedName = encodeURIComponent(
				'some name-12345678id.png',
			);
			...
			try {
				...
				expect(
					spy.calledWithMatch({ localFileName: requestedName }),
				).to.equal(true);
			} finally {
				spy.restore();
			}
			...
			const promises = folderNames.map((name) =>
				directoryService
					...
					.catch((err) => {
						...
						expect(err.code).to.equal(400);
						expect(err.message).to.equal(
							`Der Ordner '${name}' ist nicht erlaubt!`,
						);
					}),
			);
