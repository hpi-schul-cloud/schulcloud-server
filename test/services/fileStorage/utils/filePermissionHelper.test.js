	...
	describe('checkPermissions function should', () => {
		...
		after((done) => {
			const promises = [
				...
				...fixtures.homeworks.map((_) =>
					homeworkModel.findByIdAndRemove(_._id).exec(),
				),
			];
		...
		it('does not let access files for no owner, member or shared file', (done) => {
			canRead(
				'0001d224816abba584714c9c',
				'5ca613c4c7f5120b8c5bef27',
			).catch(() => {
				done();
			});
		});
		...
		it('let read, write, create and delete user created files', (done) => {
			const permissionPromises = [
				...
				canRead('0000d224816abba584714c8c', '5ca601745d629505e51252d7')
					...
					.catch(() => undefined),
				canCreate(
					'0000d224816abba584714c8c',
					'5ca601745d629505e51252d7',
				)
					.then(() => true)
					.catch(() => undefined),
				canDelete(
		...
		it('let read, write, create and delete course files for teacher', async () => {
			expect(
				canWrite(
					'0000d224816abba584714c8e',
					'5ca613c4c7f5120b8c5bef28',
				),
			).to.be.eventually.true;
			expect(
