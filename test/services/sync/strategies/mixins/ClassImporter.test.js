	...
	describe('ClassImporter', () => {
		...
		it('cannot be instantiated directly', () => {
			try {
				...
				expect(err.message).to.equal(
					'ClassImporter is not a constructor',
				);
			}
		});
		...
		it('extends a Syncer subclass with class-related operations', () => {
			...
			MIXIN_METHODS.forEach((method) =>
				expect(undecoratedInstance[method]).to.equal(undefined),
			);
			const decoratedInstance = new MixedClass();
			MIXIN_METHODS.forEach((method) =>
				expect(decoratedInstance[method]).to.not.equal(undefined),
			);
		});
		...
		describe('#findClass', () => {
			...
			it('returns null if no class was found', async () => {
				const result = await new MixedClass(app).findClass({
					foo: 'bar',
				});
			...
			it('finds classes using the class service', async () => {
				...
				const existingClass = await createTestClass({
					name: identifier,
				});
				const result = await new MixedClass(app).findClass({
					name: identifier,
				});
				expect(result._id.toString()).to.deep.equal(
		...
		describe('#createClass', () => {
			...
			it('allows creating a class', async () => {
				...
				const result = await new MixedClass(app).createClass({
					name: 'foo',
					schoolId,
				});
			...
			it('increases the number of created classes on success', async () => {
				...
				const firstClass = await instance.createClass({
					name: 'foo',
					schoolId,
				});
				expect(instance.stats.classes.created).to.equal(1);
				const secondClass = await instance.createClass({
					name: 'bar',
					schoolId,
				});
		...
		describe('#updateClass', () => {
			...
			it('allows updating a class by id', async () => {
				...
				const result = await new MixedClass(
					app,
				).updateClass(existingClass._id, { name: 'barz' });
				expect(result._id.toString()).to.deep.equal(
					existingClass._id.toString(),
				);
		...
		describe('#createOrUpdateClass', () => {
			...
			it('updates an existing class and returns it using a given query', async () => {
				...
				const instance = new MixedClass(app);
				const result = await instance.createOrUpdateClass(
					{ name: 'new name' },
					{ name: identifier },
				);
		...
		describe('#buildClassMapping', () => {
			...
			it("creates new classes if they don't exist", async () => {
				...
				const instance = new MixedClass(app);
				const { _id: schoolId } = await createTestSchool();
				const result = await instance.buildClassMapping(classes, {
					schoolId,
				});
				await Promise.all(
					classes.map(async (c) => {
			...
			it('updates existing classes of the same name and school', async () => {
				...
				expect(result['1a']._id.toString()).to.equal(
					existing1a._id.toString(),
				);
				expect(instance.stats.classes.created).to.equal(0);
				expect(instance.stats.classes.updated).to.equal(1);
				expect(instance.stats.classes.successful).to.equal(1);
