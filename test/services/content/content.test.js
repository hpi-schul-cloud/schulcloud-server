...
function requestToFilename(requestOptions) {
	// just ASCII characters, NTFS-safe
	const key = JSON.stringify(requestOptions.qs).replace(/([^ -~]|[."<>|\\/:*?])+/g, '');
	const filename = `response${key}.json`;
	return path.resolve(__dirname, 'mock', filename);
}

describe('content service', function contentServiceTest() {
