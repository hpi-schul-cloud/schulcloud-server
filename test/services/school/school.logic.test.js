...
describe('school logic', () => {
	...
	it('get default documentBaseDir', async () => {
		...
		const baseDir = String(new URL('default/', new URL(Configuration.get('DOCUMENT_BASE_DIR'))));
	...
	it('get school documentBaseDir', async () => {
		...
		const baseDir = String(new URL(String(path), new URL(Configuration.get('DOCUMENT_BASE_DIR'))));
		expect(baseDir).contains(Configuration.get('DOCUMENT_BASE_DIR'));
	...
	it('get schoolgroup documentBaseDir', async () => {
		...
		const baseDir = String(new URL(path, new URL(Configuration.get('DOCUMENT_BASE_DIR'))));
		expect(baseDir).contains(Configuration.get('DOCUMENT_BASE_DIR'));
		expect(baseDir).contains(String(path));
		expect(school.documentBaseDir).equals(baseDir);
