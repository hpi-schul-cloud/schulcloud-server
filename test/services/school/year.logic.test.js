...
describe('school year logic', async () => {
	...
	it('default years, and current/next year exist', () => {
		...
		expect(
			defaultYears.filter((year) => year.name.startsWith(nowAsYear))
				.length,
		).to.equal(
			1,
	...
	it('set custom year dates for testSchool', async () => {
		...
		customYear.startDate = customYear.startDate.setMonth(
			customYear.startDate.getMonth() + 1,
		);
		customYear.endDate = customYear.endDate.setMonth(
			customYear.endDate.getMonth() + 1,
		);
		...
		const customizedYear = facade.schoolYears.filter(
			(year) => year.name === yearToBeCustomized.name,
		)[0];
		expect(customizedYear.startDate).equals(
			customYear.startDate,
			'recently set start date does not match',
		);
		expect(customizedYear.endDate).equals(
			customYear.endDate,
			'recently set end date does not match',
		);
	});
	...
	describe('school year operations', async () => {
		...
		it('next year after year works', () => {
			...
			schoolYearFacade.schoolYears.forEach((year) => {
				...
				expect(yearNumber).to.be.greaterThan(
					lastYear,
					'values not in right order',
				);
				lastYear = yearNumber;
			});

			const nextYear = schoolYearFacade.getNextYearAfter(
				schoolYearFacade.schoolYears[0]._id,
			);
			expect(nextYear).not.to.be.null;
			expect(nextYear.name).to.equal(
				schoolYearFacade.schoolYears[1].name,
			);
		});
	});
