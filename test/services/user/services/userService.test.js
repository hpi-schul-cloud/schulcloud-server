...
describe('user service', () => {
	...
	describe('GET', () => {
		...
		it('student can not read admin email', async () => {
			...
			const result = await app
				.service('users')
				.get('0000d213816abba584714c0a', params); // admin user id
		...
		xit('student can not read student from foreign school', async () => {
			...
			const otherStudent = await testObjects.createTestUser({
				roles: ['student'],
		...
		xit('student can not read unknown student', async () => {
			...
			try {
				await app
					.service('users')
					.get('AAAAAAAAAAAAAAAAAAAAAAAAAAA', params);
				throw new Error('should have failed');
			} catch (err) {
				expect(err.message).to.not.equal('should have failed');
				expect(err.message).to.equal(testGenericErrorMessage);
		...
		xit('does not allow students to read other students without STUDENT_LIST permission', async () => {
			...
			const studentToRead = await testObjects.createTestUser({
				roles: ['student'],
			});
		...
		xit('teacher can not read other teacher', async () => {
			...
			const otherTeacher = await testObjects.createTestUser({
				roles: ['teacher'],
			});
			const params = await testObjects.generateRequestParamsFromUser(
				teacher,
		...
		xit('teacher can not read superhero', async () => {
			...
			try {
				await app
					.service('users')
					.get('0000d231816abba584714c9c', params); // superhero user id
				throw new Error('should have failed');
			} catch (err) {
				expect(err.message).to.not.equal('should have failed');
				expect(err.message).to.equal(testGenericErrorMessage);
	...
	describe('FIND', () => {
		...
		xit('does not allow population', async () => {
			...
			params.query = {
				$populate: ['0000d186816abba584714c5f', 'roles'],
			};
			try {
		...
		it('does not allow teachers to find parents', async () => {
			...
			const parent = await testObjects.createTestUser({
				roles: ['parent'],
			});

			const teacherParams = await testObjects.generateRequestParamsFromUser(
				teacher,
			);
			teacherParams.query = {};
			const result = await app.service('users').find(teacherParams);
			expect(result.data.some((r) => equalIds(r._id, parent._id))).to.be
				.false;
		});
		...
		it('does not allow students who may not create teams list other users', async () => {
			...
			const studentParams = await testObjects.generateRequestParamsFromUser(
				student,
			);
			studentParams.query = {};
			...
			await app
				.service('schools')
				.patch(student.schoolId, { enableStudentTeamCreation: false });

			try {
				await app.service('users').find(studentParams);
				assert.fail(
					'students who maynot create a team are not allowed to list other users',
				);
			} catch (error) {
		...
		it('allows students who may create teams list other users', async () => {
			...
			await app
				.service('schools')
				.patch(student.schoolId, { enableStudentTeamCreation: true });

			const studentResults = await app
				.service('users')
				.find(studentParams);
		...
		it('allows access to parents by superheroes', async () => {
			const hero = await testObjects.createTestUser({
				roles: ['superhero'],
			});
			const parent = await testObjects.createTestUser({
				roles: ['parent'],
	...
	describe('PATCH', () => {
		it('rejects on group patching', async () => {
			await userService.patch(null, { email: 'test' }).catch((err) => {
				...
				expect(err.message).to.equal(
					'Operation on this service requires an id!',
				);
			});
		});
		...
		it('student can edit himself', async () => {
			...
			const result = await app
				.service('users')
				.patch(student._id, { firstName: 'Bruce' }, params);
			expect(result).to.not.be.undefined;
		...
		it('fail to patch user on other school', async () => {
			const school = await testObjects.createTestSchool();
			const otherSchool = await testObjects.createTestSchool();
			const studentToDelete = await testObjects.createTestUser({
				roles: ['student'],
				schoolId: otherSchool._id,
			});
			const actingUser = await testObjects.createTestUser({
			...
			try {
				await app
					.service('users')
					.patch(studentToDelete._id, { lastName: 'Vader' }, params);
				throw new Error('should have failed');
			} catch (err) {
				expect(err.message).to.not.equal('should have failed');
				expect(err.code).to.equal(404);
				expect(err.message).to.equal(
					`no record found for id '${studentToDelete._id.toString()}'`,
				);
			}
		});
	});
	...
	describe('REMOVE', () => {
		it('user gets removed from classes and courses after delete', async () => {
			const userToDelete = await testObjects.createTestUser({
				roles: ['student'],
			});
			...
			expect(course.userIds.map((id) => id.toString())).to.not.include(
				testUserId.toString(),
			);
			expect(klass.userIds.map((id) => id.toString())).to.not.include(
				testUserId.toString(),
			);
		});
		...
		it('can delete single student with STUDENT_DELETE permission', async () => {
			...
			try {
				const result = await app
					.service('users')
					.remove(studentToDelete._id, params);
				expect(result).to.not.be.undefined;
				expect(result._id.toString()).to.equal(
					studentToDelete._id.toString(),
				);
			} catch (err) {
	...
	describe('bulk delete', () => {
		it('can delete multiple students when user has STUDENT_DELETE permission', async () => {
			...
			const userIds = await Promise.all([
				testObjects
					.createTestUser({ roles: ['student'], manualCleanup: true })
					.then((u) => u._id),
				testObjects
					.createTestUser({ roles: ['student'], manualCleanup: true })
					.then((u) => u._id),
			...
			userIds.forEach((userId) =>
				expect(resultUserIds).to.include(userId.toString()),
			);
		});
