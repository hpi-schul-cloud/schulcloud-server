	...
	describe('handler', () => {
		it('should accept and execute a well-defined handler function', async () => {
			...
			const fakeApp = {
				...
				use: (_path, service) =>
					(registeredService = Object.assign(service, defaults)),
				service: (path) =>
					path === SERVICE_PATH ? registeredService : defaults,
			};
			...
			const membersService = ScopeMembersService.initialize(
				fakeApp,
				SERVICE_PATH,
				handler,
			);
			...
			try {
				...
				expect.fail(
					`No error should have been thrown. Got '${err}' instead.`,
				);
			}
		});
	});
