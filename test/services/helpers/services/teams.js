...
const {
	teamsModel,
	teamUserModel,
} = require('../../../../src/services/teams/model');
...
const createTeam = () => async (owner) =>
	teamsModel
		.create({
			...
			userIds: [
				await createTeamUser(owner._id, owner.schoolId, 'teamowner'),
			],
		})
		.then((team) => {
...
const removeOneTeam = (id) =>
	teamsModel.findOneAndRemove({ _id: id }).lean().exec();
