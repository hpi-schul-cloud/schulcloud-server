...
const avgTimeToInteractiveService = app.service(
...
describe.only('insights service', () => {
	it('registers correctly', () => {
		...
		expect(app.service('insights/weeklyActiveUsers')).to.not.equal(
			undefined,
		);
		...
		expect(app.service('insights/avgTimeToInteractive')).to.not.equal(
			undefined,
		);
	...
	describe('integration tests', function integrationTests() {
		...
		insightsIntegrationTest(
			'Dau Over Mau service',
			dauOverMauService,
			objectKeys.dauOverMau,
		);
		insightsIntegrationTest(
			'Monthly Users service',
			monthlyUsersService,
			objectKeys.monthlyUsers,
		);
		insightsIntegrationTest(
			'Weekly Users service',
			weeklyUsersService,
			objectKeys.weeklyUsersService,
		);
		insightsIntegrationTest(
			'Weekly Activity service',
			weeklyActivityService,
			objectKeys.weeklyActivityService,
		);
		insightsIntegrationTest(
			'Weekly Active Users service',
			weeklyActiveUsersService,
			objectKeys.weeklyActiveUsersService,
		);
		insightsIntegrationTest(
			'Role Activity service',
			roleActivityService,
			objectKeys.roleActivityService,
		);
		insightsIntegrationTest(
			'Avg Page Loaded service',
			avgPageLoadedService,
		);
		insightsIntegrationTest(
			'Avg Time To Interactive service',
			avgTimeToInteractiveService,
		);
		insightsIntegrationTest(
			'Unique Page Count service',
			uniquePageCountService,
		);
	});
});
