	...
	describe('General login service', () => {
		...
		before(() =>
			testObjects
				.createTestUser()
				.then((testUser) =>
					testObjects.createTestAccount(testAccount, null, testUser),
				),
		);
				...
				chai.request(app)
					...
					.end((err, res) => {
						...
						accountService
							...
							.catch((error) => {
								logger.error(
									`failed to get the account from the service: ${error}`,
								);
								// throw error;
								reject(error);
								// done();
			...
			it('should be blocked if activated', () =>
				new Promise((resolve, reject) => {
					...
					testObjects
						.createTestUser()
						.then((testUser) =>
							testObjects.createTestAccount(
								testDisposableAccount,
								null,
								testUser,
							),
						)
