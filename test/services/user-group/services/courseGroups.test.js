...
describe('courseGroup service', () => {
	...
	describe('basic functions', () => {
		it('CREATE a courseGroup', async () => {
			...
			const dbResult = await courseGroupModel
				.findById(result._id)
				.lean()
				.exec();
		...
		it('GET a courseGroup', async () => {
			...
			const courseGroup = await testObjects.createTestCourseGroup({
				schoolId,
				courseId,
				userIds: [student._id],
			});
		...
		it('FIND courseGroups', async () => {
			...
			expect(result.data[0].schoolId.toString()).to.eq(
				schoolId.toString(),
			);
		});
		...
		it('PATCH a courseGroup', async () => {
			...
			const studentToAdd = await testObjects.createTestUser({
				roles: ['student'],
				schoolId,
			});
			const { _id: courseId } = await testObjects.createTestCourse({
	...
	describe('security', () => {
		...
		it('GET fails for user not in course', async () => {
			...
			try {
				...
				expect(err.message).to.equal(
					`no record found for id '${courseGroup._id}'`,
				);
			}
		});
		...
		it('UPDATE fails for user not in course', async () => {
			...
			const { _id: otherCourseId } = await testObjects.createTestCourse({
				schoolId,
				userIds: [student._id],
			});
			const courseGroup = await testObjects.createTestCourseGroup({
