...
describe('classSuccessor service', () => {
	...
	it('GET works for class without year or gradeLevel', async () => {
		...
		expect(successor.schoolId.toString()).to.equal(
			newClass.schoolId.toString(),
		);
	});
	...
	it('fails when gradeLevel is too high', async () => {
		try {
			...
			expect(err.message).to.equal(
				'there is no grade level higher than 13!',
			);
		}
	});
	...
	it('GET informs about duplicates', async () => {
		...
		expect(successor.duplicates.map((d) => d.toString())).to.include(
			oldClass._id.toString(),
		);
	});
	...
	it('only includes duplicates in the same year', async () => {
		...
		const name = `klasse ${Date.now()}`;
		const classThisYear = await testObjects.createTestClass({
			name,
			schoolId: school._id,
			year: classYear,
		});
		const classYearAfter = await testObjects.createTestClass({
			name,
			schoolId: school._id,
			year: yearAfter,
		});
	...
	it('only includes duplicates with equal grade level', async () => {
		const name = `klasse ${Date.now()}`;
		const classoldGrade = await testObjects.createTestClass({
			name,
			gradeLevel: 1,
		});
		const classNextGrade = await testObjects.createTestClass({
			name,
			gradeLevel: 2,
		});
		const newClass = await testObjects.createTestClass({
	...
	it('FIND generates multiple successors', async () => {
		const gClass = await testObjects.createTestClass({
			name: 'g',
			gradeLevel: 7,
		});
		const eClass = await testObjects.createTestClass({
			name: 'e',
			gradeLevel: 6,
		});
		const result = await classSuccessorService.find({
			query: { classIds: [gClass._id, eClass._id] },
		});
	...
	it('fails as student', async () => {
		try {
			...
			expect(error.message).to.not.equal('should have failed');
			expect(error.message).to.equal(
				"You don't have one of the permissions: CLASS_CREATE.",
			);
	...
	it('if a successor is created, the original class is updated.', async () => {
		let oldClass = await testObjects.createTestClass({
			name: 'a',
			gradeLevel: 4,
		});
	...
	it('OnClassRemove updates any existing predecessors.', async () => {
		...
		oldClass = await app
			.service('classes')
			.patch(oldClass._id, { successor: newClass._id });
		...
		await classSuccessorServiceClass.onClassRemoved.apply({ app }, [
			newClass,
		]);
		oldClass = await app.service('classes').get(oldClass._id);
