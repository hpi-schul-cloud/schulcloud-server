...
describe('lesson copy service', () => {
	...
	it('can copy a lesson with a homework', async () => {
		...
		expect(copy.courseId.toString()).to.equal(course._id.toString());
		expect(copy.description).to.eq(lesson.description);
		const homeworkCopies = await app
			.service('homework')
			.find({ query: { lessonId: copy._id } });
		expect(homeworkCopies.total).to.equal(1);
		expect(homeworkCopies.data[0]._id.toString()).to.not.equal(
			homework._id.toString(),
		);
	});

	it('can copy a lesson for a different user', async () => {
		const originalTeacher = await testObjects.createTestUser({
			roles: ['teacher'],
		});
		const targetTeacher = await testObjects.createTestUser({
			roles: ['teacher'],
		});
