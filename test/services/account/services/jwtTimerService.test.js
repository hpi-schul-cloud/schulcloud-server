		...
		describe('with redis instance', () => {
			...
			before(async () => {
				...
				delete require.cache[
					require.resolve('../../../../src/utils/redis')
				];
				delete require.cache[
					require.resolve('../../helpers/testObjects')
				];
				delete require.cache[
					require.resolve(
						'../../../../src/services/account/services/jwtTimerService',
					)
				];
				...
				} = require('../../../../src/services/account/services/jwtTimerService');
			...
			it('FIND returns the whitelist timeToLive on the JWT that is used', async () => {
				...
				await redisHelper.redisSetAsync(
					redisIdentifier,
					'value',
					'EX',
					1000,
			...
			it('CREATE resets the ttl on the jwt that is used', async () => {
				...
				const currentTtl = await redisHelper.redisTtlAsync(
					redisIdentifier,
				);
