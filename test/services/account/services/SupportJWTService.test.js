...
describe('supportJWTService', () => {
	...
	it(`create with ${testedPermission} permission should work.`, async () => {
		...
		const jwt = await supportJWTService.create(
			{ userId: student.userId },
			superhero.requestParams,
		);
		...
		expect(decodedJWT.exp <= new Date().valueOf() + expiredOffset).to.be
			.true;
	});
	...
	it(`create without ${testedPermission} permission should not work.`, async () => {
		...
		try {
			await supportJWTService.create(
				{ userId: student.userId },
