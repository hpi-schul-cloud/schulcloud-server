...
describe('consent service', () => {
	...
	it('creates consents correctly', () =>
		consentService
			...
			.then((consent) => {
				...
				chai.expect(consent.parentConsents[0]).to.have.property(
	...
	it('patches date of user consent', () =>
		consentService
			...
			.then((consent) => {
				chai.expect(consent).to.have.property('userConsent');
				chai.expect(consent.userConsent).to.have.property(
					'dateOfPrivacyConsent',
				);
			}));
	...
	it('patches instead of creating second consent for same user', () => {
		...
		return consentService
			...
			.then((results) => {
				...
				chai.expect(results.data[0].userConsent).to.have.property(
					'privacyConsent',
				);
				chai.expect(
					results.data[0].userConsent.privacyConsent,
				).to.equal(true);
			});
	});
	...
	it('finds consent versions', () =>
		consentVersionService
			...
			.then((consentVersion) => {
				...
				const specificSampleConsentVersion = consentVersion.data.filter(
					(cv) => cv.versionNumber === 'testversion',
				);
