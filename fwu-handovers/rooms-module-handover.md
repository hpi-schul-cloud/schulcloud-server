...
- School admins (`SCHOOL_ADMINISTRATE_ROOMS`) can perform member operations on rooms in their school, even without a room role
...
Invitation links allow users to join a room without being explicitly added by an admin. They are highly configurable and have their own dedicated authorization rule.
...
- An unauthenticated flow (the user may not yet be a room member)
...
checkAllowedToUseInvitationLink()
    ...
    └─► checkStudentFromOtherSchool()
        └─► Students can never join a room from a different school
```
