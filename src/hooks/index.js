...
exports.isSuperHero = () => (context) => {
	...
	return userService
		...
		.then((user) => {
			...
			if (
				!(
					user.data[0].roles.filter((u) => u.name === 'superhero')
						.length > 0
				)
			) {
	...
	return (context) => {
		...
		return app
			...
			.then(({ permissions = [] }) => {
				...
				if (!hasAnyPermission) {
					throw new Forbidden(
						`You don't have one of the permissions: ${permissionNames.join(
							', ',
						)}.`,
					);
				}
...
exports.hasSchoolPermission = (inputPermission) => async (context) => {
	...
	const results = await Promise.allSettled(
		user.roles.map(async (role) => {
			...
			throw new Forbidden(
				`You don't have one of the permissions: ${inputPermission}.`,
			);
		}),
	);
...
exports.removeResponse = (excludeOptions) => (context) => {
	...
	if (
		Array.isArray(excludeOptions) &&
		excludeOptions.includes(context.method)
	) {
		return Promise.resolve(context);
	}
...
exports.hasRoleNoHook = (context, userId, roleName, account = false) => {
	...
	if (account) {
		return accountService.get(userId).then((_account) =>
			userService
				.find({
					query: { _id: _account.userId || '', $populate: 'roles' },
				})
				.then((user) => {
					user.data[0].roles = Array.from(user.data[0].roles);

					return (
						user.data[0].roles.filter((u) => u.name === roleName)
							.length > 0
					);
				}),
		);
	...
	return userService
		.find({ query: { _id: userId || '', $populate: 'roles' } })
		.then((user) => {
			user.data[0].roles = Array.from(user.data[0].roles);

			return (
				user.data[0].roles.filter((u) => u.name === roleName).length > 0
			);
		});
};
...
exports.checkCorrectCourseOrTeamId = async (context) => {
	...
	if (courseGroupId || courseId) {
		const userId = context.params.account.userId.toString();
		// make it sense?
		const validatedCourseId =
			(courseId || '').toString() || (context.id || '').toString();
		let query = {
			teacherIds: {
				$in: [userId],
			},
		...
		const course = await context.app
			.service('courses')
			.get(validatedCourseId, { query });
...
exports.enableQueryAfter = (context) => {
	...
	if (context.result.length === 0) {
		throw new NotFound(
			`no record found for id '${context.params.query._id}'`,
		);
	}
...
exports.restrictToCurrentSchool = async (context) => {
	...
	if (
		params.route &&
		params.route.schoolId &&
		params.route.schoolId !== currentSchoolId
	) {
		throw new Forbidden(
	...
	if (['update', 'patch', 'remove'].includes(context.method) && context.id) {
		...
		if (!equalIds(target.schoolId, user.schoolId)) {
			throw new NotFound(
				`no record found for id '${context.id.toString()}'`,
			);
		}
	}
	// query
 ...
 */
const userIsInThatCourse = (
	user,
	{ userIds = [], teacherIds = [], substitutionIds = [] },
	isCourse,
) => {
	...
	return (
		userIds.some((u) => equalIds(u, userId)) ||
		testIfRoleNameExist(user, 'teacher')
	);
};
...
exports.restrictToUsersOwnLessons = (context) =>
	getUser(context).then((user) => {
		if (testIfRoleNameExist(user, ['superhero', 'administrator'])) {
			return context;
		}
		...
		if (context.type === 'before') {
			...
			if (context.method === 'get' && (context.result || {})._id) {
				...
				tempLesson = tempLesson.filter((lesson) => {
					if ('courseGroupId' in lesson) {
						return userIsInThatCourse(
							user,
							lesson.courseGroupId,
							false,
						);
					}
					return (
						userIsInThatCourse(user, lesson.courseId, true) ||
						(context.params.query.shareToken || {}) ===
							(lesson.shareToken || {})
					);
				});
				if (tempLesson.length === 0) {
					throw new Forbidden(
						"You don't have access to that lesson.",
					);
				}
				if ('courseGroupId' in context.result) {
					context.result.courseGroupId =
						context.result.courseGroupId._id;
				} else {
			...
			if (
				...
				if (context.result.data.length === 0) {
					throw new NotFound(
						'There are no lessons that you have access to.',
					);
				} else {
...
exports.restrictToUsersOwnClasses = (context) =>
	getUser(context).then((user) => {
		if (
			testIfRoleNameExist(user, ['superhero', 'administrator', 'teacher'])
		) {
			return context;
		}
...
exports.denyIfStudentTeamCreationNotAllowed = ({
	...
	const currentUserSchool = await context.app
		.service('schools')
		.get(currentUserSchoolId);
...
function validatedAttachments(attachments) {
	...
	attachments.forEach((element) => {
		...
		if (
			cTotalBufferSize >=
			Configuration.get('MAXIMUM_ALLOWABLE_TOTAL_ATTACHMENTS_SIZE_BYTE')
		) {
			throw new BadRequest(
				'Email Attachments exceed the max. total file limit.',
			);
		}
	});
}
...
exports.sendEmail = (context, maildata) => {
	...
	const roles =
		(typeof maildata.roles === 'string'
			? [maildata.roles]
			: maildata.roles) || [];
	const emails =
		(typeof maildata.emails === 'string'
			? [maildata.emails]
			: maildata.emails) || [];
	const userIds =
		(typeof maildata.userIds === 'string'
			? [maildata.userIds]
			: maildata.userIds) || [];
	const receipients = [];
	...
	files.forEach((element) => {
		try {
			attachments.push({
				...
				content: Buffer.from(element.data),
			});
		} catch (error) {
	...
	const replyEmail =
		maildata.replyEmail && re.test(maildata.replyEmail)
			? maildata.replyEmail
			: null;
	...
	if (promises.length > 0) {
			...
			.then((promise) => {
				...
				_.uniq(receipients).forEach((email) => {
					if (!maildata.content.text && !maildata.content.html) {
						logger.warning(
							"(1) No mailcontent (text/html) was given. Don't send a mail.",
						);
					} else {
						mailService
							.create({
		...
		if (!maildata.content.text && !maildata.content.html) {
			logger.warning(
				"(2) No mailcontent (text/html) was given. Don't send a mail.",
			);
		} else {
			_.uniq(receipients).forEach((email) => {
				mailService
					.create({
						...
						replyEmail,
						subject:
							maildata.subject || 'E-Mail von der Schul-Cloud',
						headers: maildata.headers || {},
						content: {
							text:
								maildata.content.text ||
...
exports.lookupSchool = async (context) => {
	if (
		...
		const { schoolId } = await context.app
			.service('users')
			.get(context.params.account.userId);
...
exports.populateCurrentSchool = async (context) => {
	if (
		...
		const { schoolId } = await context.app
			.service('users')
			.get(context.params.account.userId);
		context.params.school = await context.app
			.service('schools')
			.get(schoolId);
		return context;
	}
 ...
 * @returns {*} context
 */
exports.blockDisposableEmail = (property, optional = true) => async (
	context,
) => {
	// available
