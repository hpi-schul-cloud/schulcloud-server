...
const ES_METADATASET = Configuration.get('FEATURE_ES_MERLIN_ENABLED')  || Configuration.get('FEATURE_ES_COLLECTIONS_ENABLED')  ? 'mds_oeh' : 'mds';
const ES_ENDPOINTS = {
...
class EduSharingConnector {
	...
	async FIND({ searchQuery = '', $skip, $limit, sortProperties = 'score', collection = '' }, schoolId) {
		...
		try {
			...
			if ((searchQuery.trim().length < 2 && !collection) || (collection !== '' && !uuidV5Regex.test(collection))) {
