				...
				data.forEach((obj) => {
					...
					if (
						obj.memberOf.includes(
							this.config.providerOptions.TeacherMembershipPath,
						)
					) {
						if (!obj.givenName) {
					...
					if (
						obj.memberOf.includes(
							this.config.providerOptions.AdminMembershipPath,
						)
					) {
						if (!obj.givenName) {
							obj.givenName = 'Admin';
					...
					if (roles.length === 0) {
						const ignoredUser = obj.memberOf.some((item) =>
							this.config.providerOptions.IgnoreMembershipPath.includes(
								item,
							),
						);
						if (
