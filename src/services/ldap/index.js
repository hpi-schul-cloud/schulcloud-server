...
module.exports = function LDAPService() {
	...
	class LdapService {
		...
		get(id, params) {
			return app
				.service('systems')
				.find({ query: { _id: id }, paginate: false })
				.then((system) => {
					if (
						system[0].ldapConfig.providerOptions
							.classPathAdditions === ''
					) {
						return this.getUsers(system[0].ldapConfig, '').then(
							(userData) => ({
								users: userData,
								classes: [],
							}),
					...
					return this.getUsers(system[0].ldapConfig, '').then(
						(userData) =>
							this.getClasses(system[0].ldapConfig, '').then(
								(classData) => ({
									users: userData,
									classes: classData,
								}),
		...
		_connect(config, username, password) {
			...
			return new Promise((resolve, reject) => {
				if (!(config && config.url)) {
					reject(
						new errors.BadRequest('Invalid URL in config object.'),
					);
				}
				...
				client.bind(username, password, (err) => {
					if (err) {
						reject(
							new errors.NotAuthenticated('Wrong credentials'),
						);
					} else {
						resolve(client);
					}
		...
		authenticate(system, qualifiedUsername, password) {
			...
			return this._connect(config, qualifiedUsername, password).then(
				(connection) => {
		...
		searchObject(config, searchString, options) {
			return this.searchCollection(config, searchString, options).then(
				(objects) => {
					if (objects.length > 0) {
						return Promise.resolve(objects[0]);
					}
					return Promise.reject(
						new Error(`Object "${searchString}" not found`),
					);
				},
			);
		}
		 ...
		 */
		getExperts(config) {
			const { searchString, options } = getLDAPStrategy(
				app,
				config,
			).getExpertsQuery();
