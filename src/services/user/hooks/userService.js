...
const checkUnique = (hook) => {
	...
	if (email === undefined) {
		return Promise.reject(
			new BadRequest(
				'Fehler beim Auslesen der E-Mail-Adresse bei der Nutzererstellung.',
			),
		);
	}
	return userService
		...
		.then((result) => {
			...
			if (length === undefined || length >= 2) {
				return Promise.reject(
					new BadRequest(
						'Fehler beim Prüfen der Datenbankinformationen.',
					),
				);
			}
			if (length === 0) {
			...
			const user =
				typeof result.data[0] === 'object' ? result.data[0] : {};
			const input = typeof hook.data === 'object' ? hook.data : {};
			const isLoggedIn =
				(hook.params || {}).account && hook.params.account.userId;
			...
			if (isLoggedIn || asTask === undefined || asTask === 'student') {
				return Promise.reject(
					new BadRequest(
						`Die E-Mail Adresse ${email} ist bereits in Verwendung!`,
					),
				);
			}
			if (asTask === 'parent') {
...
const checkUniqueAccount = (hook) => {
	...
	return accountService
		.find({ query: { username: email.toLowerCase() } })
		.then((result) => {
			if (result.length > 0) {
				return Promise.reject(
					new BadRequest(
						`Ein Account mit dieser E-Mail Adresse ${email} existiert bereits!`,
					),
				);
			}
			return Promise.resolve(hook);
...
const removeStudentFromClasses = async (hook) => {
	...
	try {
		const usersClasses = await classesService.find({
			query: { userIds: { $in: userIds } },
		});
		await Promise.all(
			usersClasses.data.map((klass) =>
				classesService.patch(klass._id, {
...
const removeStudentFromCourses = async (hook) => {
	...
	try {
		const usersCourses = await coursesService.find({
			query: { userIds: { $in: userIds } },
		});
		await Promise.all(
			usersCourses.data.map((course) =>
				hook.app.service('courseModel').patch(course._id, {
					$pull: { userIds: { $in: userIds } },
				}),
			),
		);
...
const sanitizeData = (hook) => {
	if ('email' in hook.data) {
		if (!constants.expressions.email.test(hook.data.email)) {
			return Promise.reject(
				new BadRequest('Bitte gib eine valide E-Mail Adresse an!'),
			);
		}
	}
...
const pinIsVerified = (hook) => {
	...
	// eslint-disable-next-line no-underscore-dangle
	const email =
		(hook.params._additional || {}).parentEmail || hook.data.email;
	return hook.app
		...
		.then((pins) => {
			if (pins.data.length === 1 && pins.data[0].pin) {
				...
				if (
					!(
						(hook.data.roles || []).includes('student') &&
						age < CONSENT_WITHOUT_PARENTS_MIN_AGE_YEARS
					)
				) {
					hook.app
						.service('/registrationPins')
						.remove(pins.data[0]._id);
				}
			...
			return Promise.reject(
				new BadRequest(
					'Der Pin wurde noch nicht bei der Registrierung eingetragen.',
				),
			);
		});
};
...
const securePatching = async (context) => {
	const targetUser = await context.app
		.service('users')
		.get(context.id, { query: { $populate: 'roles' } });
	...
	const isDemoStudent = actingUser.roles.find(
		(r) => r.name === 'demoStudent',
	);
	const isDemoTeacher = actingUser.roles.find(
		(r) => r.name === 'demoTeacher',
	);
	...
	if (isDemoStudent || isDemoTeacher) {
		return Promise.reject(
			new Forbidden('Diese Funktion ist im Demomodus nicht verfügbar!'),
		);
	}

	if (!ObjectId.equal(targetUser.schoolId, actingUser.schoolId)) {
		return Promise.reject(
			new NotFound(`no record found for id '${context.id.toString()}'`),
		);
	}
...
const getDisplayName = (user, app) =>
	app
		...
		.then((protectedRoles) => {
			const protectedRoleIds = (protectedRoles.data || []).map(
				(role) => role._id,
			);
			const isProtectedUser = protectedRoleIds.find((role) =>
...
const decorateUser = (hook) =>
	getDisplayName(hook.result, hook.app)
		.then((displayName) => {
			hook.result =
				hook.result.constructor.name === 'model'
					? hook.result.toObject()
					: hook.result;
...
const setAvatarData = (user) => {
	...
	if (user.customAvatarBackgroundColor) {
		...
		const index =
			(user.avatarInitials.charCodeAt(0) +
				user.avatarInitials.charCodeAt(1)) %
			colors.length;
...
const enforceRoleHierarchyOnDeleteSingle = async (context) => {
	try {
		const userIsSuperhero = await hasRoleNoHook(
			context,
		...
		if (
			!permissionChecks.reduce((accumulator, val) => accumulator && val)
		) {
			throw new Forbidden(
				'you dont have permission to delete this user!',
			);
		}
...
const enforceRoleHierarchyOnDeleteBulk = async (context) => {
	...
	const roleQuery = {
		$nor: [{ roles: { $elemMatch: { $nin: allowedRoles } } }],
	};
		...
		context.data.roles.map(async (roleId) => {
			...
			if (!ObjectId.isValid(roleId)) {
				...
				try {
					...
					return Promise.reject(
						new BadRequest('No such role exists'),
					);
				}
			}
			switch (roleName) {
		...
		const { hash } = await app
			...
			.catch((err) => {
				throw new GeneralError(
					`Can not create registrationlink. ${err}`,
				);
			});
...
const filterResult = async (context) => {
	const userCallingHimself = ObjectId.equal(
		context.id,
		context.params.account.userId,
	);
	const userIsSuperhero = await hasRoleNoHook(
	...
	const elevatedUser = await hasPermissionNoHook(
		context,
		context.params.account.userId,
		'STUDENT_EDIT',
	);
...
const includeOnlySchoolRoles = async (context) => {
	if (context.params && context.params.query) {
		...
		if (context.params.query.roles && context.params.query.roles.$in) {
			...
			context.params.query.roles.$in = context.params.query.roles.$in.filter(
				(r) => allowedRoles.some((a) => ObjectId.equal(r, a)),
			);
		} else {
