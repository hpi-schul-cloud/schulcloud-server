...
const checkPermissions = async (hook) => {
	const targetUser = await hook.app
		.service('users')
		.get(hook.params.route.userId, { query: { $populate: 'roles' } });
	const actingUser = await hook.app
		.service('users')
	...
	if (
		targetIsStudent &&
		actingUser.permissions.includes('STUDENT_SKIP_REGISTRATION')
	) {
		userHasPermission = true;
	}
	if (
		targetIsTeacher &&
		actingUser.permissions.includes('TEACHER_SKIP_REGISTRATION')
	) {
		userHasPermission = true;
	}
	...
	if (!userHasPermission)
		return Promise.reject(
			new Forbidden('you do not have permission to do this!'),
		);
