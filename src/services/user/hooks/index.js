...
const checkJwt = () => function checkJwtFunction(hook) {
...
exports.before = function init(app) {
	return {
		all: [],
		find: [
...
const decorateAvatar = (hook) => {
	if (hook.result.total) {
		hook.result = (hook.result.constructor.name === 'model') ? hook.result.toObject() : hook.result;
		hook.result.data = (hook.result.data || []).map(user => setAvatarData(user));
	} else {
