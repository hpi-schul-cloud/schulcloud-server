...
/* eslint-disable prefer-promise-reject-errors */ const createParent = (
	data,
	params,
	user,
	app,
		...
		.then((check) => {
			if (
				!(
					check.data &&
					check.data.length > 0 &&
					check.data[0].pin === data.pin
				)
			) {
				return Promise.reject(
					'Ungültige Pin, bitte überprüfe die Eingabe.',
				);
			}
			const parentData = {
			...
			return app
				...
				.catch((err) => {
					...
					return Promise.reject(
						new Error('Fehler beim Erstellen des Elternaccounts.'),
					);
				});
		});
...
const firstLogin = async (data, params, app) => {
	...
	if (data['password-1']) {
		accountUpdate.password_verification =
			data.password_verification || data['password-2'];
		accountUpdate.password = data['password-1'];
		accountPromise = await app
			.service('accounts')
	...
	const userPromise = app
		.service('users')
		.patch(user._id, userUpdate, { account: params.account });
	...
	if (data.termsOfUseConsentVersion || data.privacyConsentVersion) {
		...
		updateConsentUsingVersions = app
			...
			.then((consents) => {
				...
				const updateConsentType = consent.userConsent
					? 'userConsent'
					: 'parentConsents';
				if (updateConsentType === 'userConsent') {
					updatedConsent = {
						...updatedConsent,
						...consent[updateConsentType],
					};
					updatedConsent = updateConsentDates(updatedConsent);
					return app
						.service('consents')
						.patch(consent._id, { userConsent: updatedConsent });
				}
				if (
					updateConsentType === 'parentConsents' &&
					(!consent.parentConsents || !consent.parentConsents.length)
				) {
				...
				return app
					.service('consents')
					.patch(consent._id, { parentConsents: [updatedConsent] });
			});
	}
