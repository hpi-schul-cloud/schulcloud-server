...
const populateUser = (app, data) => {
	...
	if (data.importHash) {
		return app.service('users').find({
			query: { importHash: data.importHash, _id: data.userId, $populate: ['roles'] },
		})
			.then((users) => {
...
const insertUserToDB = (app, data, user) => {
	if (user._id) {
		return app.service('users').remove(user._id)
			.then(() => app.service('users').create(
				user,
				{ _additional: { parentEmail: data.parent_email, asTask: 'student' } },
			)
				.catch((err) => {
					logger.warn(err);
	...
	return app.service('users').create(user, { _additional: { parentEmail: data.parent_email, asTask: 'student' } })
		.catch((err) => {
			...
			throw new errors.BadRequest(
				'Fehler beim Erstellen des Nutzers. Eventuell ist die E-Mail-Adresse bereits im System registriert.',
			);
		});
};

const registerUser = function init(data, params, app) {
	...
	return new Promise(((resolve, reject) => {
		...
		return Promise.all([classPromise, schoolPromise])
			.then(([classes, schools]) => {
				...
				return Promise.reject(new errors.BadRequest('Ungültiger Link'));
			});
		...
		if ((user.roles || []).includes('student')) {
			...
			if (user.birthday instanceof Date && isNaN(user.birthday)) { // enable rule
				return Promise.reject(new errors.BadRequest('Fehler bei der Erkennung des ausgewählten Geburtstages. '
				+ 'Bitte lade die Seite neu und starte erneut.'));
			}
			// wrong age?
			const age = globalHooks.getAge(user.birthday);
			if (data.parent_email && age >= 16) {
				return Promise.reject(new errors.BadRequest(`Schüleralter: ${age} Im Elternregistrierungs-Prozess darf`
				+ ' der Schüler nicht 16 Jahre oder älter sein.'));
			} if (!data.parent_email && age < 16) {
				return Promise.reject(new errors.BadRequest(`Schüleralter: ${age} Im Schülerregistrierungs-Prozess darf`
				+ ' der Schüler nicht jünger als 16 Jahre sein.'));
			}
		}
		...
		.then(() => insertUserToDB(app, data, user)
			.then((newUser) => {
				user = newUser;
			}))
		.then(() => {
			...
			if (data.sso === 'true' && data.account) {
				...
				return app.service('accounts').update(
					{ _id: accountId },
			...
			if (data.parent_email) {
				...
				return app.service('users').create(parent, { _additional: { asTask: 'parent' } })
					.catch((err) => {
						...
						return Promise.reject(new errors.GeneralError('Fehler beim Erstellen des Elternaccounts.'));
					}).then((newParent) => {
						parent = newParent;
						return userModel.userModel.findByIdAndUpdate(
							user._id,
							{ parents: [parent._id] },
							{ new: true },
						).exec()
					...
					.catch(() => Promise.reject(new errors.GeneralError('Fehler beim Verknüpfen der Eltern.')));
			}
			return Promise.resolve();
		})
		.then(() => {
		...
		.catch((err) => {
			...
			return Promise.all(rollbackPromises)
				.catch(err => Promise.reject(new errors.BadRequest(
					(err.error || {}).message
					|| err.message
					|| err
					|| 'Kritischer Fehler bei der Registrierung. Bitte wenden sie sich an den Administrator.',
				)))
				.then(() => Promise.reject(new errors.BadRequest(
					(err.error || {}).message
					|| err.message
					|| err
