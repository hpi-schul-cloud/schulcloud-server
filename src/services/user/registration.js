		...
		.then((users) => {
			if (users.data.length <= 0 || users.data.length > 1) {
				throw new errors.BadRequest(
					'Kein Nutzer für die eingegebenen Daten gefunden.',
				);
			}
	...
	if (user._id) {
		...
		return app
			...
			.catch((err) => {
				logger.warning(err);
				throw new errors.BadRequest(
					'Fehler beim Updaten der Nutzerdaten.',
				);
			});
	}
	return app
...
const registerUser = function register(data, params, app) {
	...
	return new Promise((resolve) => {
		...
		.then(() => {
			...
			classPromise = app
				.service('classes')
				.find({ query: { _id: data.classOrSchoolId } });
			schoolPromise = app
				.service('schools')
				.find({ query: { _id: data.classOrSchoolId } });
			return Promise.all([classPromise, schoolPromise]).then(
				([classes, schools]) => {
					if (classes.total === 1) {
		...
		.then(() => {
			const consentSkipCondition = Configuration.get(
		...
		.then(() => {
			...
			return app
				...
				.catch((err) => {
					const msg =
						err.message || 'Fehler wärend der Pin Überprüfung.';
		...
		.then(() => {
			...
			return app
				...
				.catch((err) =>
					Promise.reject(
						new Error('Fehler beim Erstellen des Accounts.', err),
					),
				);
		})
		.then(async (res) => {
			...
			if (data.parent_email) {
				...
				try {
					...
					logger.log(
						'warn',
						`Fehler beim Verknüpfen der Eltern. ${err}`,
					);
					return Promise.reject(
						new Error('Fehler beim Verknüpfen der Eltern.', err),
					);
				}
			}
			return Promise.resolve();
		...
		.then(() => {
			...
			if (parent) {
				...
				consentPromise = app
					.service('consents')
					.create({ userId: user._id, parentConsents: [consent] });
			} else {
				consent = {
					form: 'digital',
				...
				consentPromise = app
					.service('consents')
					.create({ userId: user._id, userConsent: consent });
			}
			return consentPromise
				...
				.catch((err) =>
					Promise.reject(
						new Error(
							'Fehler beim Speichern der Einverständniserklärung.',
							err,
						),
					),
				);
		...
		.catch((err) => {
			...
			if (parent && parent._id) {
				rollbackPromises.push(
					userModel.userModel
						.findOneAndRemove({ _id: parent._id })
						.exec(),
				);
			}
			if (account && account._id) {
				rollbackPromises.push(
					accountModel.findOneAndRemove({ _id: account._id }).exec(),
				);
			}
			if (consent && consent._id) {
				rollbackPromises.push(
					consentModel.consentModel
						.findOneAndRemove({ _id: consent._id })
						.exec(),
				);
			}
