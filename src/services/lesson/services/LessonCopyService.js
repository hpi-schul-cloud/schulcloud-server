...
class LessonCopyService {
	...
	createHomeworkCopyTask(homework, userId, newCourseId, newLesson) {
		return this.app.service('homework/copy').create({
			...
			userId:
				userId === homework.teacherId.toString()
					? userId
					: homework.teacherId,
			newTeacherId: userId,
		});
	...
	async copyHomeworks(params, { _id: oldLessonId }, newCourseId, newLesson) {
		...
		const tasks = homeworks.map((h) =>
			this.createHomeworkCopyTask(h, userId, newCourseId, newLesson),
		);
	...
	async copyFilesInLesson(params, sourceLesson, newCourseId, newLesson) {
			...
			lessonFiles.map((sourceFile) =>
				copyFile(
					...
					.catch((err) => {
						...
						fileChangeLog.push(
							sourceFile._id,
							`can_not_copyed_${sourceFile._id}`,
							sourceFile.name,
						);
		...
		return lessonModel
			.update({ _id: newLesson._id }, newLesson)
			.lean()
			.exec();
	}
