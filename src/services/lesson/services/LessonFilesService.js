...
class LessonFilesService {
	...
	replaceFileLinksByFileId(files = [], lesson) {
		...
		return files.filter((file) =>
			_.some(
				contents,
				(c) =>
					this.isTextContent(c) && this.includeFileKey(c, file.key),
			),
		);
	}
	...
	find({ lessonId, query }) {
		...
		return lessonModel
			.findOne({ _id: lessonId, shareToken })
			.then((lesson) => {
				if (!lesson) {
					throw new NotFound(
						'No lesson was not found for given lessonId and shareToken!',
					);
				}
