...
const checkIfCourseGroupLesson = (permission1, permission2, isCreating, hook) => {
	...
	const groupPromise = isCreating
		? Promise.resolve({ courseGroupId: hook.data.courseGroupId })
		: lesson.findOne({ _id: hook.id })
			.then(foundLesson => (
				JSON.stringify(foundLesson.courseGroupId)
...
const checkForShareToken = (hook) => {
	...
	return lesson.findOne({ _id: lessonId }).populate('courseId')
		.then((topic) => {
			if ((topic.shareToken === shareToken && shareToken !== undefined)
			|| topic.courseId.teacherIds.filter(t => t.toString() === hook.params.account.userId).length > 0) {
				return hook;
			}
