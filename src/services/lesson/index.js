...
class LessonFilesService {
	...
	find({ lessonId, query }) {
		...
		return lesson.findOne({ _id: lessonId, shareToken }).then((lesson) => {
			...
			return FileModel.find({ path: { $regex: lesson.courseId } }).then(files => Promise.all((files || []).filter(f => _.some((lesson.contents || []), content => content.component === 'text' && content.content.text && _.includes(content.content.text, f.key)))));
		});
	}
}
		...
		return lesson.findOne({ _id: lessonId }).populate('courseId')
			.then((sourceLesson) => {
				...
				return lesson.create(tempLesson, (err, res) => {
					...
					return FileModel.find({ path: { $regex: sourceLesson.courseId._id } }).then(files => Promise.all((files || []).filter(f => _.some((sourceLesson.contents || []), content => content.component === 'text' && content.content.text && _.includes(content.content.text, decodeURIComponent(f.key)))))
