...
const accessCheck = (consent, app) => {
	...
	return (
		app
			.service('users')
			.get(consent.userId, { query: { $populate: 'roles' } })
			...
			.then((response) => {
				...
				if (
					userHasOneRole(user, ['teacher', 'administrator', 'expert'])
				) {
				...
				if (age < CONSENT_WITHOUT_PARENTS_MIN_AGE_YEARS) {
					...
					if (
						!(
							parentConsent.privacyConsent &&
							parentConsent.termsOfUseConsent
						)
					) {
						access = false;
						return Promise.resolve();
			...
			.then(() => {
				if (
					...
					return app
						.service('users')
						.patch(user._id, { preferences: updatedPreferences });
				}
				return Promise.resolve();
			})
			.then(() => {
...
const decorateConsent = (hook) =>
	accessCheck(hook.result, hook.app)
		.then((consent) => {
			hook.result =
...
const decorateConsents = (hook) => {
	...
	const consentPromises = (hook.result.data || []).map((consent) =>
		accessCheck(consent, hook.app).then((result) => result),
	);
...
const getConsentStatus = (consent) => {
	...
	const isNOTparentConsent = (c = {}) => {
		...
		return (
			pCs.length === 0 ||
			!pCs.some((pC) => pC.privacyConsent && pC.termsOfUseConsent)
		);
	};
