const {
	ScopeMembersService,
} = require('../../helpers/scopePermissions/ScopeMembersService');
const {
	getScopePermissions,
 ...
 */
const setup = (app) => {
	const getPermissions = (course, userId) =>
		getScopePermissions(app, userId, { id: course, name: 'courses' });
	...
	const getCourseMembers = async (params) => {
		...
		const results = await Promise.all(
			ids.map((userId) =>
				getPermissions(params.scope.id, userId).then((r) => [
					userId,
					r,
				]),
			),
	...
	ScopeMembersService.initialize(
		app,
