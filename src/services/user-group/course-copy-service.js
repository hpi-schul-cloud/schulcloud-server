					...
					.then((res) => {
						...
						return Promise.all([homeworkPromise, lessonsPromise])
							.then(([homeworks, lessons]) => {
								...
								return Promise.all(lessons.map(lesson => createLesson(
									lesson._id,
									res._id,
									params.account.userId,
									this.app, lesson.shareToken,
								)
									.then((lessonRes) => {
									...
									.then(() => Promise.all(homeworks.map((homework) => {
										let convertedLesson;
										if (
											homework.archived.length > 0
											|| (homework.teacherId.toString() !== params.account.userId.toString()
												&& homework.private)
										) { return Promise.resolve(); }
										if (homework.lessonId) {
											convertedLesson = createdLessons
												.filter(h => h.name === homework.lessonId.name);
										...
										return createHomework(
											...
											params.account.userId.toString() === homework.teacherId.toString()
												? params.account.userId
												: homework.teacherId,
											this.app, params.account.userId,
										);
									}))
										.then(() => res));
							});
	...
	create(data, params) {
		...
		return courseModel.find({ shareToken })
			.then((course) => {
				...
				if (courseName) { tempCourse.name = courseName; }
