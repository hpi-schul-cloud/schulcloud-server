...
const deleteWholeClassFromCourse = (hook) => {
	...
	return CourseModel.findById(courseId)
		...
		.then((course) => {
			...
			const removedClasses = _.differenceBy(
				course.classIds,
				requestBody.classIds,
				(v) => JSON.stringify(v),
			);
			...
			return Promise.all(
				...
				hook.data.userIds = hook.data.userIds.filter(
					(value) => !studentIds.some((id) => equalIds(id, value)),
				);
				return hook;
			});
		});
...
const removeSubstitutionDuplicates = (hook) => {
	const requestBody = hook.data;
	if (
		requestBody.substitutionIds &&
		(requestBody.substitutionIds || []).length > 0
	) {
...
const courseInviteHook = async (context) => {
	if (
		...
		context.params.query.link
	) {
		// link is used as "authorization"
		const dbLink = await context.app
			.service('link')
			.get(context.params.query.link);
...
const patchPermissionHook = async (context) => {
	...
	const defaultPermissionHook = (ctx) =>
		Promise.resolve(
			checkScopePermissions(['COURSE_EDIT'])(ctx),
...
const restrictChangesToArchivedCourse = async (context) => {
	...
	if (disallowedKeys.length > 0) {
		return Promise.reject(
			new BadRequest(
				'This course is archived. To activate it, please change the end date.',
			),
		);
	}
	return context;
