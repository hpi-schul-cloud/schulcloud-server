	...
	class RegistrationLinkService {
		...
		async create(data, params) {
			...
			if (data.toHash) {
				try {
					...
					return Promise.reject(
						new Error(`Fehler beim Generieren des Hashes. ${err}`),
					);
				}
			}
			...
			if (data.role === 'student') {
				linkData.link = `${
					data.host || Configuration.get('HOST')
				}/registration/${data.schoolId}`;
			} else {
				linkData.link = `${
					data.host || Configuration.get('HOST')
				}/registration/${data.schoolId}/byemployee`;
			}
			if (linkData.hash) linkData.link += `?importHash=${linkData.hash}`;

			// remove possible double-slashes in url except the protocol ones
			linkData.link = linkData.link.replace(
				/(https?:\/\/)|(\/)+/g,
				'$1$2',
			);
		...
		create(data, params) {
			return new Promise(async (resolve) => {
				...
				if (expertSchoolId && linkInfo.hash) {
					...
					logger.warning(
						'Nicht alle Daten für den Experten-Link vorhanden.',
					);
