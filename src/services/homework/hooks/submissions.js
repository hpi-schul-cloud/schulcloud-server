	...
	if (Object.keys(hook.params.query).length === 0) {
		...
		if (hook.params.account) {
			...
			return userService.find({
				...
				message: "[500 INTERNAL ERROR] - can't reach users service",
			})));
		}
	}
	return hook;
...
const insertHomeworkData = (hook) => {
	...
	if (homeworkId) {
		...
		return homeworkService.get(homeworkId, { account: { userId: hook.params.account.userId } })
			...
			.catch(() => Promise.reject(
				new errors.GeneralError({ message: "[500 INTERNAL ERROR] - can't reach homework service" }),
			));
	}
...
const noSubmissionBefore = (hook) => {
	...
	const submissionsForMe = hook.data.submissions.filter(
		submission => (submission.teamMembers.includes(hook.params.account.userId))
			|| ((submission.studentId || {})._id === hook.params.account.userId),
	);
	if (submissionsForMe.length > 0) {
		return Promise.reject(new errors.Conflict({
			message: `${submissionsForMe[0].studentId.firstName} ${submissionsForMe[0].studentId.lastName} `
				+ 'hat bereits für dich abgegeben!',
		}));
	}
	return Promise.resolve(hook);
};
	...
	if (!hook.data.isTeacher && hook.data.teamMembers) {
		...
		if (hook.data.submission) {
			newTeamMembers = newTeamMembers
				.filter(teamMember => !hook.data.submission.teamMembers.includes(teamMember.toString()));
		}
		...
		if (submissionsForTeamMembers.length > 0) {
			return Promise.reject(new errors.Conflict({
				message: `${toRemove + ((submissionsForTeamMembers.length === 1) ? ' hat' : ' haben')}`
					+ 'bereits eine Lösung abgegeben! Entferne diese'
					+ `${(submissionsForTeamMembers.length === 1) ? 's Mitglied!' : ' Mitglieder!'}`,
			}));
		}
	}
	return Promise.resolve(hook);
...
const canGrade = (hook) => {
	if (!hook.data.isTeacher && (Number.isInteger(hook.data.grade) || typeof hook.data.gradeComment === 'string')) {
