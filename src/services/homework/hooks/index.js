...
const getAverageRating = function getAverageRating(submissions) {
	...
	if (submissions.length > 0) {
		...
		const submissiongrades = submissions.filter((s) =>
...
function isTeacher(userId, homework) {
	...
	if (!isTeacherCheck && !homework.private) {
		const isCourseTeacher = (
			homework.courseId.teacherIds || []
		).some((teacherId) => equalIds(teacherId, user));
		const isCourseSubstitution = (
			homework.courseId.substitutionIds || []
		).some((substitutionId) => equalIds(substitutionId, user));
...
const hasViewPermissionAfter = (hook) => {
	...
	function hasPermission(e) {
		const isTeacherCheck =
			...
			(!e.private &&
				((e.courseId || {}).teacherIds || []).includes(
					(hook.params.account || {}).userId.toString(),
				)) ||
			(!e.private &&
				((e.courseId || {}).substitutionIds || []).includes(
					(hook.params.account || {}).userId.toString(),
				));
		const isStudent =
			e.courseId != null &&
			((e.courseId || {}).userIds || []).includes(
				((hook.params.account || {}).userId || '').toString(),
			);
	...
	if (data[0] !== undefined) {
		...
		if (data.schoolId != undefined && !hasPermission(data)) {
			return Promise.reject(
				new errors.Forbidden("You don't have permissions!"),
			);
		}
	}
		...
		.then((submissions) => {
			data = data.map((e) => {
				...
				if (!isTeacher(hook.params.account.userId, copy)) {
					const currentSubmissions = submissions.data.filter((s) =>
						equalIds(copy._id, s.homeworkId),
					);
					const filteredSubmission = currentSubmissions.filter(
						(submission) =>
							...
							(submission.teamMembers &&
								submission.teamMembers.some((member) =>
									equalIds(
										hook.params.account.userId,
										member,
									),
					...
					const submissionWithGrade = filteredSubmission.filter(
					...
					copy.submissions = filteredSubmission.filter(
						isValidSubmission,
					).length;
				}
				...
				if (
					!copy.private &&
					((((copy.courseId || {}).userIds || []).includes(
						hook.params.account.userId.toString(),
					) &&
						copy.publicSubmissions) ||
						isTeacher(hook.params.account.userId, copy))
				) {
					const NumberOfCourseMembers = (
						(copy.courseId || {}).userIds || []
					).length;
					const currentSubmissions = submissions.data.filter((s) =>
						equalIds(copy._id, s.homeworkId),
					);
					const validSubmissions = currentSubmissions.filter(
						isValidSubmission,
					);
					const gradedSubmissions = currentSubmissions.filter(
						isGraded,
					);
					const NumberOfUsersWithSubmission = validSubmissions
						.map((e) =>
							e.courseGroupId
								? (e.courseGroupId.userIds || []).length || 1
								: (e.teamMembers || []).length || 1,
						)
						.reduce((a, b) => a + b, 0);
					...
					const submissionPerc =
						NumberOfCourseMembers !== 0
							? (NumberOfUsersWithSubmission /
									NumberOfCourseMembers) *
							  100
							: 0;
					const gradePerc =
						NumberOfCourseMembers !== 0
							? (NumberOfGradedUsers / NumberOfCourseMembers) *
							  100
							: 0;
					...
					copy.stats = {
						...
						submissionPercentage:
							submissionPerc != Infinity
								? submissionPerc.toFixed(2)
								: undefined,
	...
	return homeworkService
		...
		.then((homework) => {
			...
			const isStudent =
				...
				!!homework.courseId.userIds.find((userId) =>
					equalIds(userId, hook.params.account.userId),
				);
			// allow this student to only change archived
			const onlyChangesArchived =
				Object.keys(hook.data).length === 1 &&
			...
			if (isStudent && onlyChangesArchived) {
				...
				const removedStudents = homework.archived.filter(
					(studentId) =>
						!hook.data.archived.find((stId) =>
							equalIds(studentId, stId),
						),
				);
				...
				const addedStudents = hook.data.archived.filter(
					(studentId) =>
						!homework.archived.find((stId) =>
							equalIds(studentId, stId),
						),
				);
				const addsOnlySelf =
					addedStudents.length === 1 &&
					equalIds(addedStudents[0], hook.params.account.userId);
		...
		.catch((err) => {
			...
			return Promise.reject(
				new errors.GeneralError({
					message:
						"[500 INTERNAL ERROR] - can't reach homework service @isTeacher function",
				}),
			);
		});
};
...
exports.before = () => ({
	...
	update: [
		iff(isProvider('external'), globalHooks.hasPermission('HOMEWORK_EDIT')),
	],
	patch: [
		iff(isProvider('external'), [
