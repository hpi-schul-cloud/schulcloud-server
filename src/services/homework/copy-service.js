...
class HomeworkCopyService {
	...
	copy(copyAssignment, ignoreParams, addParams) {
		const temp = _.omit(
			JSON.parse(JSON.stringify(copyAssignment)),
			ignoreParams,
		);
	...
	async copyFilesIfExist(fileIds = [], params) {
		if (fileIds.length > 0) {
			...
			const copiedFiles = await Promise.all(
				...
				this.app.logger.error('Could not copy files for homework.', {
	...
	async get(id, params) {
		...
		const fileIds = await this.copyFilesIfExist(
			copyAssignment.fileIds,
			params,
		);
		...
		const tempAssignment = this.copy(
			copyAssignment,
			ignoredKeys,
			addingKeys,
		);
