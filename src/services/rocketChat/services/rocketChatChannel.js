...
class RocketChatChannel {
	...
	generateChannelName(teamName) {
		...
		const channelName = makeStringRCConform(
			`${teamName}.${randomSuffix()}`,
		);
		...
		return this.app
			...
			.then((team) => {
				...
				return Promise.all(userNamePromises).then(async (users) => {
					...
					const channelName =
						name ||
						(await this.generateChannelName(currentTeam.name));
					const body = {
						name: channelName,
						members: userNames,
					};
					return request(
						getRequestOptions('/api/v1/groups.create', body, true),
					).then((res) => {
						if (res.success === true) return res;
						return Promise.reject(
							new BadRequest('bad answer on group creation'),
						);
					});
				});
			})
			...
			.catch((err) => {
				logger.warning(
					new BadRequest('Can not create RocketChat Channel', err),
				);
	...
	async handleChannelMissingRcSide(err, channel, teamId) {
		...
		if (rcError === 'error-room-not-found') {
			...
			return request(
				getRequestOptions(
					`/api/v1/groups.members?roomName=${channel.channelName}`,
					{},
					true,
					{},
					'GET',
	...
	async ensureRcChannel(channel, userId, teamId) {
		const rcAccount = await this.app
			.service('/rocketChat/user')
			.get(userId);
		...
		const userInChannel = !!rcChannelMembers.members.find(
			(e) => e._id === rcAccount.rcId,
		);
		if (!userInChannel) {
			const body = {
			...
			await request(
				getRequestOptions('/api/v1/groups.invite', body, true),
			);
		}
	...
	async getOrCreateRocketChatChannel(teamId, params) {
		try {
			...
			if (!channel) {
				channel = await this.createChannel(teamId).then(() =>
					channelModel.findOne({ teamId }),
				);
			}
			...
			logger.warning(
				new BadRequest(
					'error initializing the rocketchat channel',
					err,
				),
			);
	...
	async addUsersToChannel(userIds, teamId) {
		const rcUserNames = await this.app
			.service('/rocketChat/user')
			.find({ userIds });
		const channel = await this.app
			.service('/rocketChat/channel')
			.get(teamId);
	...
	async removeUsersFromChannel(userIds, teamId) {
		...
		const kickPromises = rcUserNames.map((userName) => {
			...
			return request(
				getRequestOptions('/api/v1/groups.kick', body, true),
			).catch((err) => {
		...
		try {
			...
			if (channel) {
				...
				team.userIds.forEach(async (user) => {
					if (
						this.teamModeratorRoles.includes(user.role.toString())
					) {
						scModeratorPromises.push(
							this.app
								.service('rocketChat/user')
								.get(user.userId),
						);
				...
				const moderatorsToAdd = scModerators.filter(
					(x) => !rcChannelModerators.includes(x),
				);
				const moderatorsToRemove = rcChannelModerators.filter(
					(x) => !scModerators.includes(x),
				);
				moderatorsToAdd.forEach((x) =>
					request(
						getRequestOptions(
							'/api/v1/groups.addModerator',
							{ roomName: channel.channelName, userId: x },
							true,
						),
					),
				);
			...
			logger.warning(
				`Fehler beim Synchronisieren der rocket.chat moderatoren für team ${teamId} `,
				err,
			);
	...
	onTeamUsersChanged(context) {
		...
		let additionalUsers = (
			((context || {}).additionalInfosTeam || {}).changes || {}
		).add;
		let removedUsers = (
			((context || {}).additionalInfosTeam || {}).changes || {}
		).remove;
	...
	registerEventListeners() {
		this.app.on(
			'teams:after:usersChanged',
			this.onTeamUsersChanged.bind(this),
		); // use hook to get app
		this.app
			.service('teams')
			.on('removed', RocketChatChannel.onRemoved.bind(this));
	...
	setup(app) {
		...
		return app
			...
			.then((teamModeratorRoles) => {
				this.teamModeratorRoles = teamModeratorRoles.data.map((role) =>
					role._id.toString(),
				);
				return Promise.resolve();
			});
