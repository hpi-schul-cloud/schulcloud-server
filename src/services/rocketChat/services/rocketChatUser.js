...
class RocketChatUser {
	...
	generateUserName(user) {
		...
		const userName = makeStringRCConform(
			`${user.firstName}.${user.lastName}.${randomSuffix()}`,
		);
	...
	async handleEmailInUse(err, email, password) {
		if (err.error.error.includes('is already in use :(')) {
			...
			const rcUser = await request(
				getRequestOptions(
					`/api/v1/users.list?${queryString}`,
					{},
					true,
					undefined,
					'GET',
		...
		throw new BadRequest(
			'Can not write user informations to rocketChat.',
			err,
		);
	}
		...
		return this.app
			...
			.then(async (user) => {
				...
				const createdUser = await request(
					getRequestOptions('/api/v1/users.create', body, true),
				).catch(async (err) =>
	...
	async getOrCreateRocketChatAccount(userId) {
		try {
			const scUser = await this.app
				.service('users')
				.get(userId, { query: { $populate: 'schoolId' } });
			if (
				!(
					(scUser.schoolId.features || []).includes(
						SCHOOL_FEATURES.ROCKET_CHAT,
					) || scUser.schoolId.purpose === 'expert'
				)
			) {
				throw new BadRequest(
					'this users school does not support rocket.chat',
				);
			}
	...
	get(userId) {
		return this.getOrCreateRocketChatAccount(userId)
			...
			.catch((err) => {
				logger.warning(
					'encountered an error while fetching a rocket.chat user.',
					err,
				);
				throw err;
			});
	...
	find({ userIds }) {
		...
		if (!Array.isArray(userIds || {})) {
			return Promise.reject(
				new Forbidden('requires an array of userIds'),
			);
		}
		return Promise.all(
			userIds.map((userId) => this.getOrCreateRocketChatAccount(userId)),
		)
			.then((accounts) => {
	...
	registerEventListeners() {
		this.app
			.service('users')
			.on('removed', RocketChatUser.onUserRemoved.bind(this));
	}
