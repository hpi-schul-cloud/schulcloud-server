		...
		return userService.get(id)
			.then((user) => {
				...
				return Promise.all([
					courseService.find({
						query: {
							$or: [{ userIds: user._id }, { teacherIds: user._id }],
						},
						headers: { 'x-api-key': (params.headers || {})['x-api-key'] },
					}),
					classService.find({
						query: {
							$or: [{ userIds: user._id }, { teacherIds: user._id }],
					...
					scopes.forEach((scope) => {
						...
						const isTeacher = scope.teacherIds.filter(
							teacherId => JSON.stringify(teacherId) === JSON.stringify(user._id),
						);
...
class UserResolver {
	get(id, params) {
		...
		const getScope = Promise.all([
			...
			courseService.get(id, { headers: { 'x-api-key': (params.headers || {})['x-api-key'] } })
				.then(data => data).catch(() => undefined),
			classService.get(id, { headers: { 'x-api-key': (params.headers || {})['x-api-key'] } })
				.then(data => data).catch(() => undefined),
