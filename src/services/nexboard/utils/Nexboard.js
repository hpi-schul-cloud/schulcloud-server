...
class Nexboard {
	constructor() {
		if (
			Configuration.has('NEXBOARD_URL') &&
			Configuration.has('NEXBOARD_URI')
		) {
			this.url = () =>
				Configuration.get('NEXBOARD_URL') +
				Configuration.get('NEXBOARD_URI');
		...
		this.err = {
			...
			retrieveBoardsFromProject:
				'Could not retrieve Boards from Project.',
	...
	getProject(projectId) {
		return rp(
			this.createSettings({ endpoint: `projects/${projectId}` }),
		).catch((err) => {
