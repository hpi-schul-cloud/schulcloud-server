	...
	server.monitoring
		...
		.then((meeting = {}) => {
			...
			if (utils.isValidNotFoundResponse(response)) {
				if (create === true) {
					return server.administration.create(
						meetingName,
						meetingId,
						params,
					);
				...
				return logErrorAndThrow(
					'meeting room not found, create missing',
					response,
					NotFound,
				);
			}
		...
		.then((meeting) => {
			...
			switch (role) {
				case ROLES.MODERATOR:
					if (
						!Array.isArray(response.moderatorPW) ||
						!response.moderatorPW.length
					) {
						logErrorAndThrow(
				...
				default:
					if (
						!Array.isArray(response.attendeePW) ||
						!response.attendeePW.length
					) {
						logErrorAndThrow(
			...
			if (
				!Array.isArray(response.meetingID) ||
				!response.meetingID.length
			) {
				logErrorAndThrow('invalid meetingID', response);
			}
			return server.administration.join(
				userName,
...
exports.getMeetingInfo = (server, meetingId) =>
	server.monitoring.getMeetingInfo(meetingId).then((meeting = {}) => {
		const { response } = meeting;
		if (
			utils.isValidFoundResponse(response) ||
			utils.isValidNotFoundResponse(response)
		) {
