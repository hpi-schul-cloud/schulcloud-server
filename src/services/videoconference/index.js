...
function throweOnNotAnyPermissionInScope(requiredPermissions, scopePermissions) {
...
function userHasVideoconferencePermissionsInScope(userPermissions) {
 ...
 * @re
 */
async function getVideocenceMetadata(scopeName, scopeId, returnAsObject = false) {
...
async function updateAndGetVideoconferenceMetadata(scopeName, scopeId, options) {
	const modelDefaults = getDefaultModel(scopeName, scopeId);
	let videoconferenceSettings = await getVideocenceMetadata(scopeName, scopeId);
...
class GetVideoconferenceService {
	...
	async get(scopeId, params) {
		...
		const videoconferenceMetadata = (await getVideocenceMetadata(scopeName, scopeId, true));
...
class CreateVideoconferenceService {
	...
	async create(data = {}, params) {
		...
		// CHECK PERMISSIONS //////////////////////////////////////////////////////
		await throwOnFeaturesDisabled(authenticatedUser);
		throweOnNotAnyPermissionInScope([
		...
		try {
			...
			if (hasStartPermission) {
				...
				// (hasJoinPermission)
				videoconferenceMetadata = (await getVideocenceMetadata(scopeName, scopeId, true));
