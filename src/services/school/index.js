...
module.exports = function schoolServices() {
	...
	const ADMIN_TOGGLE_STUDENT_VISIBILITY = Configuration.get(
	...
	if (ADMIN_TOGGLE_STUDENT_VISIBILITY !== 'disabled') {
		app.use(
			...
			new HandlePermissions('teacher', 'STUDENT_LIST'),
		);
		const handlePermissionsService = app.service(
			'/school/teacher/studentvisibility',
		);
