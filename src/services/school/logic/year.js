class SchoolYearFacade {
	...
	constructor(years, school) {
		 ...
		 */
		const customYearsOf = (yearId) =>
			(school.customYears || []).filter(
		...
		if (school) {
			this.customYears = (school.customYears || []).sort(
				SchoolYearFacade.yearCompare,
			);
			this.schoolYears = generateSchoolYears();
			if (school.currentYear) {
				this.activeYearIndex = this.schoolYears.findIndex((year) =>
					year._id.equals(school.currentYear),
				);
			}
		} else {
	...
	get activeYear() {
		...
		const now = Date.now();
		const activeYears = this.schoolYears.filter(
			(year) => year.startDate <= now && year.endDate >= now,
		);
	...
	get nextYear() {
		if (
			this.activeYearIndex != null &&
			this.activeYearIndex + 1 < this.schoolYears.length
		) {
			return this.schoolYears[this.activeYearIndex + 1];
		}
		const now = Date.now();
		const nextYears = this.schoolYears.filter(
			(year) => year.startDate >= now,
		);
