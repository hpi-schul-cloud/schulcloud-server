...
const updateSchoolPermissions = (ctx, schoolId, permissions) =>
	ctx.service('schools').patch(schoolId, { permissions });
...
class HandlePermissions {
	...
	async find(params) {
		...
		const schoolPermision = ((school.permissions || [])[this.role] || [])[
			this.permission
		];
		let isEnabled = false;
	...
	async patch(id, data, params) {
		...
		if (
			!Object.prototype.hasOwnProperty.call(schoolPermissions, this.role)
		) {
