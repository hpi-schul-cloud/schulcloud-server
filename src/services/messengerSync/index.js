...
const {
	messengerSchoolSyncService,
	messengerSchoolSyncHooks,
} = require('./services/schoolSyncService');
const {
	messengerTokenService,
	messengerTokenHooks,
} = require('./services/messengerTokenService');
...
module.exports = (app) => {
	if (Configuration.get('FEATURE_MATRIX_MESSENGER_ENABLED')) {
		...
		app.service('schools/:schoolId/messengerSync').hooks(
			messengerSchoolSyncHooks,
		);
