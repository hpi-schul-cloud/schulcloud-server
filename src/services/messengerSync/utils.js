...
const buildTeamObject = async (team, userId, moderatorRoles) => {
	...
	const isModerator = team.userIds.some((user) => {
		...
		return [teamAdminId, teamLeaderId, teamOwnerId].includes(
			user.role.toString(),
		);
	});
	return {
...
const buildMessageObject = async (data) => {
	...
	if (data.teams) {
		await Promise.all(
			data.teams.map(async (team) => {
				const teamObject = await buildTeamObject(
					team,
					data.userId,
					moderatorRoles,
				);
	...
	return {
		...
		school: {
			...
			has_allhands_channel: (school.features || []).includes(
		...
		user: {
			...
			email: user.email,
			is_school_admin: user.roles.some((roleId) =>
				ObjectId.equal(roleId, moderatorRoles.adminRoleId),
			),
			is_school_teacher: user.roles.some((roleId) =>
				ObjectId.equal(roleId, moderatorRoles.teacherRoleId),
			),
		},
		rooms,
	};
