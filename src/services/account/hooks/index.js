...
const validateCredentials = async (hook) => {
	...
	const client = await systemStrategy.credentialCheck(
		username,
		password,
		system,
	);
...
const validatePassword = (hook) => {
	...
	return Promise.all([
		globalHooks.hasPermissionNoHook(
			hook,
		...
		([
			...
			const userDidFirstLogin =
				user.preferences && user.preferences.firstLogin;
			...
			if (passwordVerification) {
				return new Promise((resolve, reject) => {
					bcrypt.compare(
						...
						hook.params.account.password,
						(err, res) => {
							if (err) {
								reject(
									new BadRequest(
										'Ups, bcrypt ran into an error.',
									),
								);
							}
							if (!res) {
	...
	return accountService
		...
		.then((result) => {
			...
			if (filtered.length > 0) {
				return Promise.reject(
					new BadRequest('Der Benutzername ist bereits vergeben!'),
				);
			}
...
const checkExistence = (hook) => {
	...
	return accountService.find({ query: { userId } }).then((result) => {
		...
		if (filtered.length > 0)
			return Promise.reject(
				new BadRequest('Der Account existiert bereits!'),
			);
		return Promise.resolve(hook);
	});
};
...
const protectUserId = (hook) => {
	...
	if (hook.data.userId) {
		return accountService.get(hook.id).then((res) => {
			...
			return Promise.reject(
				new Forbidden('Die userId kann nicht geändert werden.'),
			);
		});
	}
	return hook;
		...
		([
			...
			if (hook.params.account._id !== hook.id) {
				if (isDemoStudent || isDemoTeacher) {
					return Promise.reject(
						new Forbidden(
							'Diese Funktion ist im Demomodus nicht verfügbar!',
						),
					);
				}
				if (
...
const restrictToUsersSchool = async (context) => {
	const userIsSuperhero = await globalHooks.hasRoleNoHook(
		context,
	...
	const { schoolId: usersSchoolId } = await userService.get(
		context.params.account.userId,
	);

	const targetAccount = await context.app
		.service('accountModel')
		.get(context.id);
	const { schoolId: targetSchoolId } = await userService.get(
