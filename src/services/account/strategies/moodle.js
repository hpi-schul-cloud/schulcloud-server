...
class MoodleLoginStrategy extends AbstractLoginStrategy {
	...
	login({ username, password }, system) {
		...
		if (!moodleOptions.username) return Promise.reject(new errors.NotAuthenticated('no username set'));
		if (!moodleOptions.password/* && !moodleOptions.token */) {
			return Promise.reject(new errors.NotAuthenticated('No password set')); // TODO:'or token'.
		}
		if (!moodleOptions.wwwroot) {
			return Promise.reject(new errors.NotAuthenticated('no url for moodle login provided'));
		}
	...
	updateAccount(accountId, moodleClientInstance) {
		const accountService = this.app.service('/accounts');
		return accountService.patch(accountId, { token: moodleClientInstance.token });
	}
}
