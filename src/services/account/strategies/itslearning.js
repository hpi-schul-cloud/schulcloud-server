...
class ITSLearningLoginStrategy extends AbstractLoginStrategy {
	login({ username, password }, system) {
		...
		if (!itsLearningOptions.username) return Promise.reject(new errors.NotAuthenticated('No username set'));
		if (!itsLearningOptions.password) return Promise.reject(new errors.NotAuthenticated('No password set'));
		if (!itsLearningOptions.wwwroot) {
			return Promise.reject(new errors.NotAuthenticated('No url for ITSLearning login provided'));
		}
		const childArgs = [
			path.join(__dirname, '/utils/itslearning_phantom.js'),
			itsLearningOptions.username,
			itsLearningOptions.password,
			itsLearningOptions.wwwroot,
		];
	...
	getParameterByName(name, url) {
		...
		name = name.replace(/[\[\]]/g, '\\$&'); // ToDo: check if escape character is needed
