	...
	ENABLE_THUMBNAIL_GENERATION
		? Promise.all([
			...
			rp
				...
				.catch((err) => {
					logger.warning(
						new Error('Can not create tumbnail', err),
					); // todo err message is lost and throw error
				}),
		  )
		: Promise.resolve();
				...
				{ _id: file._id },
				{
					$set: {
						'securityCheck.status':
							SecurityCheckStatusTypes.WONTCHECK,
				...
				rp.post({
					...
					body: {
						...
						callback_uri: url.resolve(
							FILE_SECURITY_CHECK_CALLBACK_URI,
							file.securityCheck.requestToken,
						),
					},
					json: true,
				}),
...
const fileStorageService = {
	...
	async create(data, params) {
		...
		const asyncErrorHandler = (err = {}) => {
			const message =
				err.message ||
				'Error during async file operation after upload.';
		...
		if (parent) {
			return canCreate(userId, parent)
				...
				.then((file) => {
					prepareSecurityCheck(file, userId, strategy).catch(
						asyncErrorHandler,
					);
					prepareThumbnailGeneration(
						file,
						strategy,
						userId,
						data,
	...
	find({ query, payload }) {
		...
		const parentPromise = parent
			? canRead(userId, parent).catch(() =>
				Promise.reject(new Forbidden()),
			  )
			: Promise.resolve();
	...
	remove(id, { query, payload }) {
		...
		return fileInstance
			...
			.then(([file]) =>
				createCorrectStrategy(fileStorageType).deleteFile(
					userId,
		...
		const permissionPromise = () => {
			...
			if (teamObject) {
				return new Promise((resolve, reject) => {
					const teamMember = teamObject.userIds.find((u) =>
						equalIds(u.userId, userId),
					);
					if (teamMember) {
						return resolve();
					}
...
const signedUrlService = {
	 ...
	 */
	create(
		{ parent, filename, fileType, flatFileName: _flatFileName },
		params,
	) {
		const {
			payload: { userId },
			...
			.then(() => {
				if (this.fileRegexCheck(filename)) {
					throw new BadRequest(
						`Die Datei '${filename}' ist nicht erlaubt!`,
					);
				}
...
const directoryService = {
	...
	async create(data, params) {
		...
		if (!sendPermissions) {
			const teamObject = await teamsModel
				.findOne({ _id: owner })
				.lean()
				.exec();
		...
		const props = sanitizeObj(
			Object.assign(data, {
				...
				refOwnerModel,
				creator: userId,
				permissions: [...permissions, ...sendPermissions].map(
					this.setRefId,
				),
			}),
		);
		...
		return this.directoryExists({
			...
			_data ? Promise.resolve(_data) : FileModel.create(props),
		);
	},
	...
	async find({ query, payload }) {
		...
		const getScopeListService = (scopeName) =>
			this.app.service(`/users/:scopeId/${scopeName}`);
	...
	create(data, params) {
		...
		return canWrite(userId, _id)
			...
			.then((obj) => {
				if (!obj) {
					return new NotFound(
						'The given directory/file was not found!',
					);
				}
...
const filePermissionService = {
	...
	async find({ query, payload }) {
		...
		// deprecated: author check via file.permissions[0].refId is deprecated and will be removed in the next release
		const isFileCreator = equalIds(
			fileObj.creator || fileObj.permissions[0].refId,
			userId,
		);
