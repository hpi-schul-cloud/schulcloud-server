 ...
 */
const delteFilesThatUserCanAccess = async (userId) => {
	try {
		...
		return {
			delta: true,
			data: [], // [{ _id, permissions: [ {} ]} , { _id, permissions: [ {} ]}]
		};
	} catch (err) {
...
/* TODO in additional ticket
try {
	// const ids = context.data.map(({ _id }) => _id);
// delete referenzes
// this by the delete task
...
const deleteUserData = async (userId) => {
	...
	const results = await Promise.allSettled([delteFilesThatUserCanAccess(userId)]);
