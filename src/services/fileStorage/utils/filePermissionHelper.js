...
const getFile = (id) =>
	FileModel.findOne({ _id: id }).populate('owner').lean().exec();
...
const checkTeamPermission = async ({ user, file, permission }) => {
	...
	return new Promise((resolve, reject) => {
		...
		while (rolesToTest.length > 0 && rolePermissions === undefined) {
			...
			rolePermissions = permissions.find((perm) =>
				equalIds(perm.refId, roleId),
			);
		...
		const findRole = (roleId) => (roles) =>
			roles.findIndex((r) => equalIds(r._id, roleId)) > -1;
...
const checkMemberStatus = ({ file, user }) => {
	...
	return [userIds, teacherIds, substitutionIds].reduce(
		(result, list) => result || (list && list.find(finder)),
		false,
	);
};
...
const checkPermissions = (permission) => async (user, file) => {
	...
	const userPermissions = permissions.find(
		(perm) => perm.refId && perm.refId.toString() === user.toString(),
	);
	...
	const submissionPromise = Submission.findOne({
		...
		.lean()
		.exec();
	const homeworkPromise = Homework.findOne({ fileIds: fileObject._id })
		.populate('courseId')
		.lean()
		.exec();
	...
	if (refOwnerModel === 'course' || submission) {
		const userObject = await userModel
			...
			.lean()
			.exec();
		const isStudent = userObject.roles.find(
			(role) => role.name === 'student',
		);
		let courseFile = fileObject;
		let submissionHomework;
		if (submission) {
			submissionHomework = await Homework.findOne({
			...
			courseFile = {
				...fileObject,
				owner: submissionHomework.courseId || {},
			};
		}
