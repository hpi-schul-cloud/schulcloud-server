...
const FEATURE_MULTIPLE_S3_PROVIDERS_ENABLED = Configuration.get(
...
class AWSS3Strategy extends AbstractFileStorageStrategy {
		...
		return new Promise((resolve, reject) => {
			...
			awsObject.s3.headBucket(params, (err) => {
				if (err && err.statusCode === 404) {
					logger.info(
						`Bucket ${awsObject.bucket} does not exist - creating ... `,
					);
	...
	getFiles(userId, path) {
		...
		if (!userId || !path) {
			return Promise.reject(
				new BadRequest('Missing parameters by getFiles.'),
			);
		}
	...
	copyFile(userId, oldPath, newPath, externalSchoolId) {
		if (!userId || !oldPath || !newPath) {
			return Promise.reject(
				new BadRequest('Missing parameters by copyFile.', {
					userId,
					oldPath,
					newPath,
				}),
			...
			.then((result) => {
				...
				return createAWSObject(result.schoolId).then((awsObject) => {
					...
					const sourceBucket = `bucket-${
						externalSchoolId || result.schoolId
					}`;
					...
					return promisify(
						awsObject.s3.copyObject.bind(awsObject.s3),
						awsObject.s3,
					)(params);
				});
			})
	...
	deleteFile(userId, filename) {
		if (!userId || !filename) {
			return Promise.reject(
				new BadRequest('Missing parameters by deleteFile.', {
					userId,
					filename,
				}),
			);
		...
		return UserModel.userModel
			...
			.then((result) => {
				...
				return createAWSObject(result.schoolId).then((awsObject) => {
					...
					return promisify(
						awsObject.s3.deleteObjects.bind(awsObject.s3),
						awsObject.s3,
					)(params);
				});
			});
	...
	generateSignedUrl({ userId, flatFileName, fileType, header }) {
		if (!userId || !flatFileName || !fileType) {
			return Promise.reject(
				new BadRequest('Missing parameters by generateSignedUrl.', {
					userId,
					flatFileName,
					fileType,
				}),
			...
			.then((result) => {
				...
				return createAWSObject(result.schoolId).then((awsObject) =>
					this.createIfNotExists(awsObject).then((safeAwsObject) => {
						...
						return promisify(
							safeAwsObject.s3.getSignedUrl.bind(
								safeAwsObject.s3,
							),
							safeAwsObject.s3,
						)('putObject', params);
	...
	getSignedUrl({
		...
		if (!userId || !flatFileName) {
			return Promise.reject(
				new BadRequest('Missing parameters by getSignedUrl.', {
					userId,
					flatFileName,
				}),
			);
		...
		return UserModel.userModel
			...
			.then((result) => {
				...
				return createAWSObject(result.schoolId).then((awsObject) => {
					...
					return promisify(
						awsObject.s3.getSignedUrl.bind(awsObject.s3),
						awsObject.s3,
					)(action, params);
				});
			});
	...
	createDirectory(userId, path) {
		...
		if (!userId || !path) {
			return Promise.reject(
				new BadRequest('Missing parameters by createDirectory'),
			);
		}
		return filePermissionHelper
			.checkPermissions(userId, path)
			.then((res) => {
				...
				return UserModel.userModel
					...
					.then((result) => {
						...
						return createAWSObject(result.schoolId).then(
							(awsObject) => {
								...
								return promisify(
									awsObject.s3.putObject.bind(awsObject.s3),
									awsObject.s3,
								)(params);
							},
						);
	...
	deleteDirectory(userId, path) {
		...
		if (!userId || !path) {
			return Promise.reject(
				new BadRequest('Missing parameters by deleteDirectory.'),
			);
		}
		return filePermissionHelper
			.checkPermissions(userId, path)
