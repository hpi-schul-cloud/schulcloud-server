...
const createAWSObject = schoolId => {
	if (!awsConfig.endpointUrl)
		throw new Error('AWS integration is not configured on the server');
...
const getFileMetadata = (storageContext, awsObjects, bucketName, s3) => {
	...
	return Promise.all(
		awsObjects.map(object => {
			return headObject({ Bucket: bucketName, Key: object.Key }).then(
				res => {
					return {
						key: object.Key,
	...
	create(schoolId) {
		...
		return SchoolModel.schoolModel
			...
			.then(result => {
				...
				return createBucket({ Bucket: awsObject.bucket }).then(res => {
	...
	copyFile(userId, oldPath, newPath, externalSchoolId) {
		...
		return UserModel.userModel
			...
			.then(result => {
				...
				const awsObject = createAWSObject(result.schoolId);
				let sourceBucket = `bucket-${externalSchoolId ||
		...
		return promisify(awsObject.s3.listObjectsV2, awsObject.s3)(params)
			.then(data => {
				if (data.Contents.length == 0)
					throw new Error(`Invalid Prefix ${params.Prefix}`); // there should always be at least the .scfake file
