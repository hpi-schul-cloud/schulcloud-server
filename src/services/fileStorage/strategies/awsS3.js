...
//	awsConfig = require(`../../../../config/secrets.${prodMode ? 'js' : 'json'}`).aws;
	...
	(['production'].includes(process.env.NODE_ENV)) // todo: fix linter issues
...
class AWSS3Strategy extends AbstractFileStorageStrategy {
		...
		return new Promise((resolve, reject) => {
			...
			awsObject.s3.headBucket(params, (err) => {
				if (err && err.statusCode === 404) {
					...
					awsObject.s3.createBucket({ Bucket: awsObject.bucket }, (bucketError) => {
						if (bucketError) {
							reject(bucketError);
						}
	...
	getSignedUrl({
		userId, flatFileName, download, action = 'getObject',
	}) {
