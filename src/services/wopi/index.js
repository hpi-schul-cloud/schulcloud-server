		...
		return FileModel.findOne({ _id: fileId })
			...
			.then((canWriteBool) => {
				...
				return Promise.resolve(
					Object.assign(
						hostCapabilitiesHelper.defaultCapabilities(),
						capabilities,
					),
				);
			})
	...
	create(data, { payload, _id, account, wopiAction }) {
		...
		return FileModel.findOne({ _id })
			...
			.then((file) => {
				...
				return filePostActionHelper(wopiAction)(
					file,
					payload,
					account,
					this.app,
	...
	create(data, params) {
		...
		if (wopiAction !== 'PUT') {
			throw new BadRequest(
				'WopiFilesContentsService: Wrong X-WOPI-Override header value!',
			);
		}
...
module.exports = function setup() {
	...
	app.use(
		`${wopiPrefix}:fileId/contents`,
		new WopiFilesContentsService(app),
		handleResponseHeaders,
	);
	app.use(
		`${wopiPrefix}:fileId`,
		new WopiFilesInfoService(app),
		handleResponseHeaders,
	);
