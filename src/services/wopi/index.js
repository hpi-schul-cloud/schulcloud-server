 ...
 * returns information about a file, a user’s permissions on that file, and general information about the capabilities
 * that the WOPI host has on the file.
...
class WopiFilesInfoService {
	...
	find({ fileId, account }) {
		...
		return FileModel.findOne({ _id: fileId })
			...
			.then((canWriteResult) => {
				capabilities = {
					...capabilities,
					UserCanWrite: Boolean(canWriteResult),
	...
	create(data, {
		payload, _id, account, wopiAction,
	}) {
		// check whether a valid file is requested
...
class WopiFilesContentsService {
	...
	*/
	create(data, {
		fileId, payload, account, wopiAction,
	}) {
		if (wopiAction !== 'PUT') {
			throw new errors.BadRequest('WopiFilesContentsService: Wrong X-WOPI-Override header value!');
		}
