...
const wopiAuthentication = (hook) => {
	...
	let jwt =
		(hook.params.query || {}).access_token ||
		hook.params.headers.authorization; // depends on client
	if (!jwt) {
...
const retrieveWopiOverrideHeader = (hook) => {
	...
	if (!hook.params.headers['x-wopi-override']) {
		throw new BadRequest(
			'X-WOPI-Override header was not provided or was empty!',
		);
	}
	hook.params.payload.wopiRequestedName =
		hook.params.headers['x-wopi-requestedname'];
...
const checkLockHeader = (hook) => {
	...
	return FileModel.findOne({ _id: fileId }).then((file) => {
		...
		if (fileLockId && fileLockId !== lockId) {
			throw new Conflict(
				'Lock mismatch: The given file could be locked by another wopi-client!',
			);
		}
...
const setLockResponseHeader = (hook) => {
	hook.result.headerPipes = [
		{ key: 'X-WOPI-Lock', value: hook.result.lockId || '' },
	];
	return hook;
};
