...
/**
 * handles the authentication for wopi-clients, the wopi-specific param 'access-token' has to be a valid jwt for the
 * current system
 *
 * Excerpt from official documentation: http://wopi.readthedocs.io/projects/wopirest/en/latest/concepts.html
 * "Note that WOPI clients are not required to pass the access token in the Authorization header, but they must send
 * it as a URL parameter in all WOPI operations. Thus, for maximum compatibility, WOPI hosts should either use the
 * URL parameter in all cases, or fall back to it if the Authorization header is not included in the request."
 * @param {*} hook
 */
...
const retrieveWopiOverrideHeader = (hook) => {
	...
	if (!hook.params.headers['x-wopi-override']) {
		throw new errors.BadRequest('X-WOPI-Override header was not provided or was empty!');
	}
...
const checkLockHeader = (hook) => {
	...
	return FileModel.findOne({ _id: fileId }).then((file) => {
		...
		if (fileLockId && fileLockId !== lockId) {
			throw new errors.Conflict('Lock mismatch: The given file could be locked by another wopi-client!');
		}
