...
class UserRoles {
	async get(id, params) {
		...
		const [roles, school] = await Promise.all([
			this.app
				.service('roles')
				.find({ query: { _id: { $in: user.roles } } }),
			this.app.service('schools').get(user.schoolId),
		]);
		const permissions = await Promise.all(
			roles.data.map((role) => rolesModel.resolvePermissions(role.roles)),
		);
		roles.data.forEach((role, i) => {
			role.permissions = [
				...new Set([...role.permissions, ...permissions[i]]),
			];
		});
