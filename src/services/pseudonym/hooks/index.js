	...
	return hook.app
		...
		.then((tool) => {
			hook.params.query.toolId =
				tool.originTool || hook.params.query.toolId;
			return hook;
		});
};
	...
	return context.app
		...
		.then((currentUserCourses) => {
			for (const course of currentUserCourses.data) {
				courseUserIds = courseUserIds.concat(
					course.userIds,
					course.teacherIds,
				...
				for (const classInstance of course.classIds || []) {
					courseUserIds = courseUserIds.concat(
						classInstance.userIds,
						classInstance.teacherIds,
					);
				}
			}
			courseUserIds = new Set(
				courseUserIds.map((element) => element.toString()),
			);
			context.result.data = context.result.data.filter((pseudonym) =>
				courseUserIds.has(pseudonym.userId.toString()),
			);
			return context;
		});
};
