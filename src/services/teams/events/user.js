...
const getTeams = (userId) =>
	teamsModel.find({ 'userIds.userId': userId }).lean().exec();
const patchTeamUsers = (team) =>
	teamsModel
		.findByIdAndUpdate(team._id, { userIds: team.userIds })
		.lean()
		.exec();
const removeTeam = (team) =>
	teamsModel.findByIdAndRemove(team._id).lean().exec();
		...
		await Promise.all(promises)
			...
			.catch((err) => {
				const notRemovedFromTeams = teams.filter((team) =>
					team.userIds.some((teamUser) =>
						equalIds(teamUser.userId, userId),
					),
				);
