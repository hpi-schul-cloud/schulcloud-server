 ...
 * @example filterToRelated(['_id','userIds'], 'result.data') in hook.result.data
 * all keys are removed that are not _id, or userIds
 */
 ...
 * @ifNotLocal
 */
const hasTeamPermission = (inputPermissions, _teamId) => globalHooks.ifNotLocal((hook) => {
	...
	return Promise.all(
		[teamRolesToHook(hook), getTeam(hook)],
	).then(([ref, team]) => {
		if (get(hook, 'isSuperhero') === true) {
			return Promise.resolve(hook);
		}
		...
		permissions.forEach((_permission) => {
			if (userTeamPermissions.includes(_permission) === false) {
				throw new Forbidden(`No permission=${_permission} found!`);
			}
		});
...
const denyRoleToInviteExtern = userRole => globalHooks.ifNotLocal(
	hook => globalHooks.hasNotRole(hook, bsonIdToString(hook.params.account.userId), userRole)
		.catch((err) => {
			logger.warn(new Error(err));
			throw new Forbidden('No permission to invite extern');
		}),
);

exports.denyRoleToInviteExtern = denyRoleToInviteExtern;
...
const testChangesForPermissionRouting = globalHooks.ifNotLocal((hook) => {
	...
	return Promise.all([
		...
		let isLeaveTeam = false; let isRemoveOthers = false;
		...
		changes.add.forEach((e) => {
			const user = users.find(entry => isSameId(e.userId, entry._id));
		...
		hook.data.userIds.forEach((_) => {
			const teamUser = team.userIds.find(id => isSameId(_.userId, id.userId));
...
const addCurrentUser = globalHooks.ifNotLocal((hook) => {
	if (hasKey(hook.result, 'userIds')) {
		...
		if (isUndefined([user, user.role], 'OR')) {
			logger.warn('Can not execute addCurrentUser for unknown user. '
...
const keys = {
	...
	data: ['filePermission', 'name', 'times', 'description', 'userIds', 'color',
		'features', 'ltiToolIds', 'classIds', 'startDate', 'untilDate', 'schoolId'],
};
...
exports.beforeExtern = {
	all: [
		...
		denyRoleToInviteExtern('student'),
	],
	find: [],
	get: [],
