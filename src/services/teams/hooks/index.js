...
const testChangesForPermissionRouting = globalHooks.ifNotLocal((hook) => {
	...
	return Promise.all([
		...
		hook.data.userIds.forEach((_) => {
			const teamUser = team.userIds.find(u => isSameId(_.userId, u.userId));
...
const keys = {
	...
	data: ['filePermission', 'name', 'times', 'description', 'userIds', 'color', 'features', 'ltiToolIds', 'classIds',
		'startDate', 'untilDate', 'schoolId'],
};
