	...
	Promise.all([
		...
		.then(([sessionUser, team, users]) => {
			...
			if (isUndefined(isAccept)) {
				const userExist = team.userIds.some((_user) =>
					isSameId(_user.userId, userId),
				);
				const schoolExist = team.schoolIds.includes(sessionSchoolId);

				if (isUndefined([userExist, schoolExist], 'OR')) {
					throw new Forbidden(
						'You have not the permission to access this. (1)',
						{ userExist, schoolExist },
					);
				}
			}
		...
		const filter = (data) => {
			...
			if (Array.isArray(data)) {
				out = data.map((element) =>
					keysArr.reduce(reducer(element), {}),
				);
			} else {
	...
	return hook.app
		...
		.then((roles) => {
			...
			hook.findRole = (key, value, resultKey) => {
				...
				const role = self.teamroles.find(
					(teamroles) =>
						teamroles[key].toString() === value.toString(),
				);
				let out;
				if (role && resultKey) {
					out = role[resultKey];
	...
	globalHooks.ifNotLocal((hook) => {
		...
		return Promise.all([
			...
			.then(([, ref, team]) => {
				...
				const teamUser = team.userIds.find((_user) =>
					isSameId(_user.userId, userId),
				);

				if (isUndefined(teamUser)) {
					throw new NotFound(
						`Session user is not in this team userId=${userId} teamId=${teamId}`,
					);
				}

				const teamRoleId = teamUser.role;
				const userTeamPermissions = ref.findRole(
					'_id',
					teamRoleId,
					'permissions',
				);
				...
				permsissions.forEach((_permsission) => {
					if (userTeamPermissions.includes(_permsission) === false) {
						throw new Forbidden(
							`No permission=${_permsission} found!`,
						);
					}
				});
			...
			.catch((err) => {
				logger.warning(err);
				throw new Forbidden(
					'You have not the permission to access this. (2)',
				);
			});
	});
...
const rejectDefaultFilePermissionUpdatesIfNotPermitted = (context) => {
	...
	const updatesDefaultFilePermissions = isDefined(
		context.data.filePermission,
	);
...
const isHigherOrEqualTeamrole = (hook, firstId, secondId) => {
	...
	const secondRole =
		secondId !== '' ? hook.findRole('_id', secondId, 'name') : '';
	return (
		teamRoleHierarchy.indexOf(firstRole) >=
		teamRoleHierarchy.indexOf(secondRole)
	);
};
...
const testChangesForPermissionRouting = globalHooks.ifNotLocal(async (hook) => {
	...
	if (
		isDefined(
			[d.times, d.color, d.description, d.name, d.startDate, d.untilDate],
			'OR',
		)
	) {
	...
	return Promise.all([
		...
		.then(([team, users]) => {
			const changes = arrayRemoveAddDiffs(
				team.userIds,
				hook.data.userIds,
				'userId',
			); // remove add
			...
			hook.data.userIds.forEach((_) => {
				const teamUser = team.userIds.find((tu) =>
					isSameId(_.userId, tu.userId),
				);
				if (isDefined(teamUser)) {
					if (isDefined(_.role) && !isSameId(teamUser.role, _.role)) {
						hasChangeRole = true;
						if (
							isHigherOrEqualTeamrole(
								hook,
								_.role,
								highestChangedRole,
							)
			...
			if (isLeaveTeam) {
				wait.push(
					leaveTeam(hook).catch(() => {
						throw new Forbidden(
							'Permission LEAVE_TEAM is missing.',
						);
					}),
				);
			}
			...
			if (isRemoveOthers) {
				wait.push(
					removeMembers(hook).catch(() => {
						throw new Forbidden(
							'Permission REMOVE_MEMBERS is missing.',
						);
					}),
				);
			}
			...
			if (isAddingFromOwnSchool) {
				wait.push(
					addSchoolMembers(hook).catch(() => {
						throw new Forbidden(
							'Permission ADD_SCHOOL_MEMBERS is missing.',
						);
					}),
				);
			}
			...
			if (hasChangeRole) {
				wait.push(
					changeTeamRoles(hook).catch(() => {
						throw new Forbidden(
							'Permission CHANGE_TEAM_ROLES is missing.',
						);
					}),
				);

				const sessionUserTeamUser = team.userIds.find((user) =>
					equalIds(user.userId, sessionUserId),
				);
				const sessionUserTeamRole = (
					sessionUserTeamUser || {}
				).role.toString();
				if (
					!isHigherOrEqualTeamrole(
					...
					wait.push(
						Promise.reject(
							new Forbidden(
								'You cant change a Permission higher than yours',
							),
						),
					);
				}
		...
		.catch((err) => {
			logger.warning(err);
			throw new Forbidden(
				'You have not the permission to access this. (4)',
			);
		});
});
		...
		hook.app
			...
			.then((school) => {
				...
				if (
					...
					if (
						roleNames.includes('student') &&
						!school.isTeamCreationByStudentsEnabled
					) {
						throw new Forbidden(
							'Your school admin does not allow team creations by students.',
						);
					}
				} else {
					throw new Forbidden(
		...
		if (!isArrayWithElement(hook.result.userIds)) {
			out = hook.app
				...
				.catch((err) => {
					logger.warning(err);
					throw new Conflict(
						'It want to remove the team with no user, but do not found it.',
					);
				});
		}
	} else {
