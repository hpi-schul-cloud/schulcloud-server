...
module.exports = {
	tokenIsActive: (context) =>
		context.app
			...
			.create({
				token: context.params.headers.authorization.replace(
					'Bearer ',
					'',
				),
			})
	...
	userIsMatching: (context) => {
		if (
			context.params.tokenInfo.obfuscated_subject ===
				decodeURIComponent(context.params.route.user) ||
	...
	stripIframe: (context) => {
		...
		context.params.pseudonym = pseudonym.includes('iframe')
			? pseudonym.match(regEx)[1]
			: pseudonym;
		return context;
	},
			...
			.then((originTools) =>
				toolService
					...
					.then((tools) => {
						...
						context.params.toolIds = context.params.toolIds.concat(
							tools.data.map((tool) => tool._id),
						);
						return context;
					}),
			);
	...
	groupContainsUser: (context) => {
		...
		const users = context.result.data.students.concat(
			context.result.data.teachers,
		);
		if (
			users.find(
				(user) =>
