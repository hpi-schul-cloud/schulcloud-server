	...
	class ClassImporter extends superClass {
		...
		async createOrUpdateClass(classObject, query, onlyAddNew = false) {
			...
			try {
				...
				if (existing) {
					if (onlyAddNew) {
						options.teacherIds = Array.from(
							new Set(
								(existing.teacherIds || []).concat(
									options.teacherIds,
								),
							),
						);
						options.userIds = Array.from(
							new Set(
								(existing.userIds || []).concat(
									options.userIds,
								),
							),
						);
		...
		async updateClass(id, options) {
			...
			try {
				...
				this.logError(
					`Failed to patch class '${id}' with options ${JSON.stringify(
						options,
					)}`,
					err,
				);
