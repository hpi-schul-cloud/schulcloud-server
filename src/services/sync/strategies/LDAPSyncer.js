...
class LDAPSyncer extends SystemSyncer {
		...
		return Promise.all([
			...
			if (years.total === 0 || states.total === 0) {
				return Promise.reject(
					new errors.GeneralError('Database should contain at least one year and one valid federal state'),
				);
			}
	...
	createSchoolsFromLdapData(data) {
		...
		let newSchools = 0;
		let updates = 0;
		return Promise.all(data.map(ldapSchool => this.app.service('schools')
			.find({ query: { ldapSchoolIdentifier: ldapSchool.ldapOu } })
			.then((schools) => {
				if (schools.total !== 0) {
					updates += 1;
