...
class LDAPSchoolSyncer extends SystemSyncer {
	...
	async getUserData() {
		...
		const ldapUsers = await this.app
			.service('ldap')
			.getUsers(this.system.ldapConfig, this.school);
		this.logInfo(`Creating and updating ${ldapUsers.length} users...`);
		const jobs = ldapUsers.map((idmUser) => async () =>
	...
	async getClassData() {
		...
		const classes = await this.app
			.service('ldap')
			.getClasses(this.system.ldapConfig, this.school);
		...
		return this.app
			...
			.then((users) => {
				if (users.total !== 0) {
					...
					return this.checkForUserChangesAndUpdate(
						idmUser,
						users.data[0],
					);
				}
	...
	async createClassesFromLdapData(data, school) {
		const jobs = data.map((ldapClass) => async () => {
			try {
				const klass = await this.getOrCreateClassFromLdapData(
					ldapClass,
					school,
				);
				...
				this.logError('Cannot create synced class', {
					error: err,
					data,
				});
			}
	...
	async getOrCreateClassFromLdapData(data, school) {
		...
		return this.app
			.service('classes')
			.patch(existingClass._id, { name: data.className });
	}
	...
	async populateClassUsers(ldapClass, currentClass) {
		...
		if (students.length > 0 || teachers.length > 0) {
			await this.app.service('classes').patch(currentClass._id, {
				userIds: students,
				teacherIds: teachers,
			});
		}
