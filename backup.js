...
const getTimestamp = () => {
	const d = new Date();
	return `${d.getUTCFullYear()}_${`0${d.getUTCMonth() + 1}`.slice(
		-2,
	)}_${`0${d.getUTCDate()}`.slice(-2)}_${`0${d.getUTCHours()}`.slice(
		-2,
	)}_${`0${d.getUTCMinutes()}`.slice(-2)}_${`0${d.getUTCSeconds()}`.slice(
		-2,
	)}`;
};
...
const cleanJoin = (a = [], seperator = ' ') =>
	a.filter(Boolean).join(seperator);
...
const importCollection = async ({ collection, filePath, drop = true }) => {
	const cmdArgs = [
		...
		collection,
		filePath,
		(await getFirstCharFromFile(filePath)) === '['
			? '--jsonArray'
			: undefined,
		drop ? '--drop ' : undefined,
	];
